...
vm.runInContext(`
  ...
  const openingPopulationCap=enemyPopulationCap(false,false),savedPopulation=state.enemies;if(openingPopulationCap<68||openingPopulationCap>82||openingPopulationCap>=MAX_ENEMIES)throw new Error('opening encounter population budget is invalid');state.enemies=Array.from({length:openingPopulationCap},()=>({dead:false,boss:false}));if(spawnEnemyGroup(6).length!==0)throw new Error('encounter director exceeded its active population budget');state.enemies=savedPopulation;
