...
vm.runInContext(`
  ...
  const openingFormation=encounterFormation(40,0),lateFormation=encounterFormation(400,5);if(!openingFormation.types.includes('charger')||!lateFormation.types.includes('anchor')||!lateFormation.types.includes('reaper'))throw new Error('surge formations do not evolve with the run');const formationSavedEnemies=state.enemies,formationSurges=state.encounterSurges,formationEliteT=state.eliteT;state.enemies=[];state.time=40;state.spawnT=99;state.eliteT=999;state.spawnEncounterPhase='BUILD';spawnWave(.1);const deployedSurge=state.encounterSurges;if(deployedSurge!==formationSurges+1||!state.enemies.some(enemy=>enemy.type==='charger')||!document.getElementById('rewardRibbonTitle').textContent.includes('HOSTILE SURGE'))throw new Error('surge phase did not deploy a readable contact formation');spawnWave(.1);if(state.encounterSurges!==deployedSurge)throw new Error('surge formation repeated inside one encounter phase');state.enemies=formationSavedEnemies;state.encounterSurges=formationSurges;state.eliteT=formationEliteT;state.time=0;state.spawnT=0;state.spawnEncounterPhase='OPENING';
