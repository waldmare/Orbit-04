...
vm.runInContext(`
  ...
  if(!document.getElementById('audioStatusText').textContent.includes('DISABLED')) throw new Error('audio status did not report disabled output');
  ...
  testAudioOutput(); if(save.settings.audio!=='ON'||sampleAttempts===0||fallbackStarts===0||!document.getElementById('audioStatusText').textContent.includes('READY')) throw new Error('audio output recovery failed'); save.settings.audio='OFF';
