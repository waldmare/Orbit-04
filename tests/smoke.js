...
const requiredAssets=[
  ...
  'assets/visuals/player-last-ark-v2.png','assets/visuals/enemy-void-larva-v1.png','assets/visuals/enemy-ossuary-v1.png','assets/visuals/enemy-witness-v1.png','assets/visuals/boss-conquest-leviathan-v1.png',
...
vm.runInContext(`
  ...
  if(document.getElementById('framePreview').dataset.shipAsset!==ENGINE_ASSETS.player||document.getElementById('framePreview').dataset.previewSource!=='runtime-player-hull'||document.getElementById('framePreview').dataset.frame!==save.selected)throw new Error('hangar preview is not bound to the deployed player hull');
  const beforeHangar={credits:save.credits,selected:save.selected,unlocked:[...save.unlocked]};
  save.credits=0;previewFrame('bastion');if(save.selected!==beforeHangar.selected||save.credits!==0||unlockPreviewFrame()||!document.getElementById('frameUnlockBtn').disabled)throw new Error('locked preview must not select or buy an unaffordable frame');
  save.credits=SHIPS.bastion.cost;previewFrame('bastion');if(save.credits!==SHIPS.bastion.cost||save.unlocked.includes('bastion'))throw new Error('preview spent credits without unlock action');
  if(!unlockPreviewFrame()||save.credits!==0||save.selected!=='bastion'||!save.unlocked.includes('bastion')||unlockPreviewFrame())throw new Error('explicit unlock must charge exactly once and select the frame');
  document.getElementById('frameFilterBtn').onclick();if(document.getElementById('shipGrid').children.length!==save.unlocked.length)throw new Error('owned frame filter leaked locked cards');document.getElementById('frameFilterBtn').onclick();if(document.getElementById('shipGrid').children.length!==Object.keys(SHIPS).length)throw new Error('all frames filter did not restore the roster');
  save.credits=beforeHangar.credits;save.selected=beforeHangar.selected;save.unlocked=beforeHangar.unlocked;previewFrameId='';renderMenu();
  ...
  if(state.p.visualAccent!==HANGAR_PALETTES[state.p.frame].accent||state.p.visualEdge!==HANGAR_PALETTES[state.p.frame].edge)throw new Error('deployed frame effects do not match the hangar palette');
  ...
  const openingPopulationCap=enemyPopulationCap(false,false),savedPopulation=state.enemies;if(openingPopulationCap<98||openingPopulationCap>110||openingPopulationCap>=MAX_ENEMIES)throw new Error('opening encounter population budget is invalid');const openingRhythm=encounterRhythm(5),surgeRhythm=encounterRhythm(40),releaseRhythm=encounterRhythm(50),bossRhythm=encounterRhythm(210,true,false);if(openingRhythm.phase!=='OPENING'||surgeRhythm.phase!=='SURGE'||surgeRhythm.density<=1||releaseRhythm.phase!=='RELEASE'||releaseRhythm.density>=1||bossRhythm.phase!=='BOSS')throw new Error('encounter rhythm does not provide build, surge, release, and boss phases');state.enemies=Array.from({length:openingPopulationCap},()=>({dead:false,boss:false}));if(spawnEnemyGroup(6).length!==0)throw new Error('encounter director exceeded its active population budget');state.enemies=[];state.time=30;state.spawnT=0;const densityRandom=Math.random;Math.random=()=>0;spawnWave(.1);Math.random=densityRandom;if(state.enemies.length!==2||state.encounterPhase!=='BUILD')throw new Error('opening encounter cadence did not admit an early hostile pair');state.enemies=savedPopulation;state.time=0;state.spawnT=0;
  const openingFormation=encounterFormation(40,0),lateFormation=encounterFormation(400,0),lateAlternate=encounterFormation(400,2);if(!openingFormation.types.includes('charger')||!lateFormation.types.includes('anchor')||!lateFormation.types.includes('reaper')||lateAlternate===lateFormation)throw new Error('surge formations do not evolve through the latest available contact groups');const formationSavedEnemies=state.enemies,formationSurges=state.encounterSurges,formationEliteT=state.eliteT;state.enemies=[];state.time=40;state.spawnT=99;state.eliteT=999;state.spawnEncounterPhase='BUILD';spawnWave(.1);const deployedSurge=state.encounterSurges;if(deployedSurge!==formationSurges+1||!state.enemies.some(enemy=>enemy.type==='charger')||!document.getElementById('rewardRibbonTitle').textContent.includes('HOSTILE SURGE'))throw new Error('surge phase did not deploy a readable contact formation');spawnWave(.1);if(state.encounterSurges!==deployedSurge)throw new Error('surge formation repeated inside one encounter phase');state.enemies=formationSavedEnemies;state.encounterSurges=formationSurges;state.eliteT=formationEliteT;state.time=0;state.spawnT=0;state.spawnEncounterPhase='OPENING';
  ...
  const dashEnd=state.p.x; if(tryPhaseDash()||state.p.x!==dashEnd) throw new Error('PHASE DASH cooldown failed'); keys.d=false;state.p.dashCooldown=0;keys.d=true;const riposteShots=Array.from({length:3},(_,index)=>({x:state.p.x+38,y:state.p.y+(index-1)*8,r:3,life:2,damage:1,grazed:false,vx:0,vy:0}));state.enemyBullets.push(...riposteShots);if(!tryPhaseDash()||state.phaseRipostes!==1||state.p.dashCooldown>2.21||state.surgeUntil<=state.time||riposteShots.some(shot=>shot.life>0)||!document.getElementById('rewardRibbonTitle').textContent.includes('PHASE RIPOSTE'))throw new Error('swept-path dash riposte reward failed');keys.d=false;
  ...
  if(state.generatedCells.size<20||!state.worldSites.length||!state.worldNodes.length||!state.worldSites.some(site=>site.kind==='planet'&&site.active)) throw new Error('procedural world or visible planetary scenery did not initialize');
  ...
  state.enemies=[];const lockNear=spawnEnemy('scout',false,{x:state.p.x+70,y:state.p.y}),lockElite=spawnEnemy('tank',true,{x:state.p.x+180,y:state.p.y});lockNear.speed=lockElite.speed=0;save.settings.targetPriority='NEAREST';if(updateTargetLock()!==lockNear||state.lockedTarget!==lockNear||state.targetLockPulse<.29)throw new Error('nearest auto-target lock failed');save.settings.targetPriority='ELITES FIRST';if(updateTargetLock()!==lockElite||state.lockedTarget!==lockElite||state.targetLockPulse<.29)throw new Error('elite-priority auto-target lock failed');lockNear.dead=lockElite.dead=true;if(updateTargetLock()!==null||state.lockedTarget!==null)throw new Error('auto-target lock did not clear');save.settings.targetPriority='NEAREST';state.enemies=[];
  ...
  save.settings.audio='ON';AUDIO.music(state.time,{fracture:true,time:state.time});if(!musicRates['audio-music_fracture']||musicRates['audio-music_fracture']>.65||Object.values(musicRates).some(rate=>rate>.65)) throw new Error('Time Fracture did not slow the loaded soundtrack');const fractureDirector=AUDIO.status();if(fractureDirector.musicScene!=='FRACTURE'||fractureDirector.primaryMusicKey!=='fracture'||fractureDirector.musicPlaying!==1)throw new Error('Time Fracture music scene did not take exclusive focus');AUDIO.music(58,{time:58,threat:2});AUDIO.music(116,{time:116,threat:3,drive:1});const musicDirector=AUDIO.status();if(musicDirector.musicPassage!==2||musicDirector.musicRotationCount<3)throw new Error('soundtrack passage rotation did not advance across the run');if(musicDirector.musicScene!=='EXPLORE'||musicDirector.primaryMusicKey!=='exploration'||musicDirector.musicPlaying>2)throw new Error('adaptive soundtrack did not maintain a clean foreground mix');AUDIO.music(195,{time:195,bossApproach:true,threat:2});const approachDirector=AUDIO.status();if(approachDirector.musicScene!=='PRESSURE'||approachDirector.primaryMusicKey!=='pressure'||approachDirector.musicPlaying>2)throw new Error('boss approach did not receive a controlled musical escalation');AUDIO.sfx('enemyShot'); const gatedSampleCount=sampleAttempts; AUDIO.sfx('enemyShot'); if(sampleAttempts!==gatedSampleCount) throw new Error('repetitive audio voice limiting failed');AUDIO.sfx('pulse');const weaponGroupCount=sampleAttempts;AUDIO.sfx('drone');if(sampleAttempts!==weaponGroupCount)throw new Error('weapon bus did not suppress same-frame routine clutter');AUDIO.sfx('rail');if(sampleAttempts<=weaponGroupCount)throw new Error('priority weapon cue was incorrectly suppressed');
