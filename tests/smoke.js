...
vm.runInContext(`
  ...
  const dashEnd=state.p.x; if(tryPhaseDash()||state.p.x!==dashEnd) throw new Error('PHASE DASH cooldown failed'); keys.d=false;state.p.dashCooldown=0;keys.d=true;const riposteShots=Array.from({length:3},(_,index)=>({x:state.p.x+82,y:state.p.y+(index-1)*8,r:3,life:2,damage:1,grazed:false,vx:0,vy:0}));state.enemyBullets.push(...riposteShots);if(!tryPhaseDash()||state.phaseRipostes!==1||state.p.dashCooldown>2.21||state.surgeUntil<=state.time||riposteShots.some(shot=>shot.life>0)||!document.getElementById('rewardRibbonTitle').textContent.includes('PHASE RIPOSTE'))throw new Error('skillful dash riposte reward failed');keys.d=false;
