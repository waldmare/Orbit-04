...
assert.equal(pkg.version,'0.66.0');
...
for(const token of ['tryPhaseDash','dashCooldown','damageNumber','telegraphs','audioMix','MIXES','musicDuck','SIGNAL RUSH','desiredBackground','licensed-sample-assets-v1','cinematicLayers','selectEnemyType','followWorldCamera','translateWorld','unlockSamples','played===false','testAudioOutput'])assert.ok(game.includes(token),`missing runtime integration token: ${token}`);
...
for(const token of ['abilityBar','ambientDrift','panelArrival','eventImpact','rushField','data-motion','intelLayout','settingsPresets','audioCheckRow'])assert.ok(styles.includes(token),`missing presentation token: ${token}`);
...
console.log('ORBIT 0.66.0 top-down runtime integration: PASS');
