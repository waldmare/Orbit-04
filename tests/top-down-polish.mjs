...
const html=read('index.html'),game=read('game.js'),visuals=read('visual-engine.js'),styles=read('styles.css'),pkg=JSON.parse(read('package.json'));
...
for(const token of ['tryPhaseDash','dashCooldown','damageNumber','telegraphs','audioMix','MIXES','musicDuck','SIGNAL RUSH','desiredBackground','licensed-cinematic-assets-v1','cinematicLayers'])assert.ok(game.includes(token),`missing polished gameplay system: ${token}`);
...
for(const token of ['abilityBar','ambientDrift','panelArrival','eventImpact','rushField','data-motion'])assert.ok(styles.includes(token),`missing presentation polish: ${token}`);
...
for(const asset of ['laserSmall_002.ogg','explosionCrunch_004.ogg','forceField_001.ogg','laserLarge_001.ogg','lowFrequency_explosion_001.ogg']){
  const full=path.join(root,'assets/audio/premium/kenney-sci-fi-sounds',asset);assert.ok(existsSync(full)&&statSync(full).size>7000,`missing premium audio layer: ${asset}`);
}
...
console.log('ORBIT 0.63 premium visuals/audio: PASS');
