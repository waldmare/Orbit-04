...
assert.match(docs,/version 0\.86\.0 Last Ark runtime capture[\s\S]*captured from the active 0\.86\.0 Electron\/WebGL build/,'current runtime screenshot is not labeled accurately');
for(const token of ['--orbit-capture','--orbit-menu-capture','--orbit-steam-capture','orbit04-capture','waitForRenderer','capturePage','runtime-screenshot-v0.86.0.png','launch-hangar-v0.86.0.png','horizontalOverflow','image.isEmpty','setPermissionRequestHandler','requestSingleInstanceLock'])assert.ok(desktop.includes(token),`missing runtime capture or desktop safeguard: ${token}`);
...
const screenshot=path.join(root,'docs/runtime-screenshot-v0.86.0.png'),png=readFileSync(screenshot);
...
const menuScreenshot=path.join(root,'docs/launch-hangar-v0.86.0.png'),menuPng=readFileSync(menuScreenshot);
...
for(const token of ['abilityBar','ambientDrift','panelArrival','eventImpact','rushField','data-motion','intelLayout','settingsPresets','audioCheckRow','navigationSignal','archivePanel','rewardRibbon','hudMetric','directiveTracker','frameStage','frameLoadout','runOption','profileSummary'])assert.ok(styles.includes(token),`missing presentation token: ${token}`);
...
console.log('ORBIT 0.86.0 focused hangar and archive integrity: PASS');
