...
for(const token of ['WebGLRenderer','OrthographicCamera','ACESFilmicToneMapping','SRGBColorSpace','DoubleSide','premultipliedAlpha:true','three-r185-topdown','aces-topdown-v2-hidpi','syncRendererResolution','devicePixelRatio','getMaxAnisotropy','ResizeObserver','pickupIconTexture','pickupTextures','pickupKind','signalColors','enemyRoleTexture','enemyRoleTextures','enemyAccent','enemyBodyTint','markedEnemyTypes','enemyMarkers','enemyRings','enemySilhouettes','enemyWakes','bulletSilhouettes','hostileBulletOutlines','movementHeading','turnCompression','visualYaw','surgeStretch','engineBias','playerRim','playerCore','playerAttitudeLeft','playerAttitudeRight','organicSway','speedStretch','fireRecoil','smoothValue','smoothAngle','smoothMotion','motionFor','playerEngineLeft','playerEngineRight','playerTrail','allyTrails','engineTexture','syncPlayer','syncEnemies','syncAllies','syncProjectiles','syncOrbitals','syncTransientFx','syncDeathFx','syncFloaters','weapon-muzzle-premium-v1.png','weapon-projectile-premium-v1.png','pools={enemies','weaponKick','dashFx','AdditiveBlending'])assert.ok(threeVisuals.includes(token),`missing active Three.js presentation feature: ${token}`);
...
for(const token of ['abilityBar','ambientDrift','panelArrival','eventImpact','rushField','data-motion','intelLayout','settingsPresets','audioCheckRow','navigationSignal','archivePanel','rewardRibbon','hudMetric','directiveTracker','frameStage','frameLoadout','runOption','profileSummary','profileDetails','optionStepper','optionIcon','launchSummary','uiIcon','hm-cyan','hm-magenta','hm-acid','renderStatus'])assert.ok(styles.includes(token),`missing presentation token: ${token}`);

for(const asset of ['pulse.wav','pulse_alt.wav','missile.wav','rail.wav','flak.wav','rift.wav','enemy_shot.wav','enemy_heavy_shot.wav','system_install.wav','achievement.wav','artifact.wav','graze.wav','streak.wav','sub_bass.wav','engine_loop.wav']){
  const full=path.join(root,'assets/audio',asset);assert.ok(existsSync(full)&&statSync(full).size>50000,`missing layered studio audio asset: ${asset}`);const wav=readFileSync(full);assert.equal(wav.subarray(0,4).toString(),'RIFF',`${asset} is not PCM WAV`);assert.equal(wav.readUInt16LE(22),2,`${asset} must be stereo`);assert.equal(wav.readUInt32LE(24),48000,`${asset} must be 48 kHz`);
}
