...
assert.match(docs,/version 0\.83\.0 Last Ark runtime capture[\s\S]*captured from the active 0\.83\.0 Electron\/WebGL build/,'current runtime screenshot is not labeled accurately');
for(const token of ['--orbit-capture','--orbit-steam-capture','orbit04-capture','waitForRenderer','capturePage','runtime-screenshot-v0.83.0.png','image.isEmpty','setPermissionRequestHandler','requestSingleInstanceLock'])assert.ok(desktop.includes(token),`missing runtime capture or desktop safeguard: ${token}`);
...
const screenshot=path.join(root,'docs/runtime-screenshot-v0.83.0.png'),png=readFileSync(screenshot);
...
console.log('ORBIT 0.83.0 tactical QoL runtime integration: PASS');
