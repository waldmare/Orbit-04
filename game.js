...
const DEFAULT_SETTINGS={playerHp:'BOTH',enemyHp:'ELITES',xpReadout:'BOTH',shake:'ON',flash:'ON',particles:'HIGH',audio:'ON'};
const DEFAULT_STATS={totalDamage:0,damageTaken:0,bosses:0,highestLevel:1,bestChain:0,totalGrazes:0,clears:0,hardlineClears:0,blackoutClears:0,playTime:0,caches:0,conversions:0};
const DEFAULT_SAVE={credits:0,bestScore:0,bestTime:0,totalKills:0,runs:0,unlocked:['striker'],selected:'striker',difficulty:'STANDARD',achievements:[],research:{},discoveredWeapons:['pulse'],discoveredEnemies:['scout'],stats:{...DEFAULT_STATS},settings:{...DEFAULT_SETTINGS}};
function deepProfile(raw={}){return {...DEFAULT_SAVE,...raw,unlocked:Array.isArray(raw.unlocked)?raw.unlocked:['striker'],achievements:Array.isArray(raw.achievements)?raw.achievements:[],research:{...(raw.research||{})},discoveredWeapons:Array.isArray(raw.discoveredWeapons)?raw.discoveredWeapons:['pulse'],discoveredEnemies:Array.isArray(raw.discoveredEnemies)?raw.discoveredEnemies:['scout'],stats:{...DEFAULT_STATS,...(raw.stats||{})},settings:{...DEFAULT_SETTINGS,...(raw.settings||{})}}}
function loadSave(){
...
const AUDIO=(()=>{
  ...
  function sfx(name){if(!enabled())return;const now=performance.now();switch(name){
    ...
    case'enemyShot':if(now-lastEnemyShot>80){lastEnemyShot=now;tone(300,.035,'square',.011,-80)}break;case'kill':if(now-lastKill>35){lastKill=now;tone(105,.04,'triangle',.014,-45)}break;case'pickup':if(now-lastPickup>52){lastPickup=now;tone(880,.022,'square',.01,180)}break;
    ...
    case'overdrive':tone(330,.06,'sawtooth',.034,260);tone(660,.08,'square',.03,340,.06);break;case'graze':tone(1180,.022,'triangle',.009,100);break;case'streak':tone(980,.03,'square',.018,160);tone(1320,.04,'square',.015,120,.03);break;
  ...
  function music(time,boss=false){if(!enabled()||!ensure())return;if(time+.02<nextBeat)return;const step=boss?.30:.375,roots=boss?[82.41,98,110,123.47]:[110,130.81,146.83,164.81],root=roots[(Math.floor(beat/4))%roots.length];if(beat%2===0)tone(root,.11,'triangle',.026,-4,0,'music');if(beat%4===0)tone(root*2,.05,'square',.014,root*.12,0,'music');if(beat%8===6)tone(root*3,.04,'square',.009,-root*.35,0,'music');beat++;nextBeat+=step;if(nextBeat<time-.5)nextBeat=time}
...
function renderMenu(){
  ...
  const d=DIFFICULTIES[save.difficulty]||DIFFICULTIES.STANDARD;$('difficultyBtn').textContent=d.name;$('difficultyDesc').textContent=d.desc;
  ...
  for(const [id,s] of Object.entries(SHIPS)){
    ...
    b.innerHTML=`<div><div class="shipName">${s.name}</div><div class="tiny" style="margin-top:5px">${s.desc}</div></div><div><div class="stat"><span>HULL</span><span>${s.hp}</span></div><div class="stat"><span>SPEED</span><span>${s.speed}</span></div><div class="stat"><span>START</span><span>${WEAPON_META[s.weapon].name}</span></div>${unlocked?'':`<div class="shipLock">UNLOCK — ${cost} CR</div>`}</div>`;
...
function renderCodex(){
  $('codexStatsTab').classList.toggle('selected',codexTab==='stats');$('codexWeaponsTab').classList.toggle('selected',codexTab==='weapons');$('codexEnemiesTab').classList.toggle('selected',codexTab==='enemies');const c=$('codexContent');c.innerHTML='';
  $('codexSummary').textContent=`RUNS ${save.runs} · CLEARS ${save.stats.clears}`;
  if(codexTab==='stats'){
    const rows=[['BEST SCORE',Math.floor(save.bestScore)],['BEST TIME',fmtTime(save.bestTime)],['TOTAL KILLS',save.totalKills],['TOTAL DAMAGE',Math.floor(save.stats.totalDamage)],['BOSSES',save.stats.bosses],['BEST CHAIN',save.stats.bestChain],['GRAZES',save.stats.totalGrazes],['CACHES',save.stats.caches],['CONVERSIONS',save.stats.conversions],['HIGHEST LEVEL',save.stats.highestLevel],['PLAY TIME',fmtTime(save.stats.playTime)],['RESEARCH',researchLevels()]];
    ...
    for(const [id,w] of Object.entries(WEAPON_META)){const known=save.discoveredWeapons.includes(id),d=document.createElement('div');d.className='codexCard';d.innerHTML=`<b>${known?w.name:'UNKNOWN SYSTEM'}</b><span>${known?`${w.desc}<br>EVOLUTION: ${w.evo}`:'Acquire this weapon during a run.'}</span>`;c.appendChild(d)}
  }else{
...
const SETTING_CYCLES={playerHp:['OFF','HUD','BAR','BOTH'],enemyHp:['OFF','ELITES','ALL'],xpReadout:['OFF','VALUE','PERCENT','BOTH'],shake:['ON','OFF'],flash:['ON','OFF'],particles:['HIGH','LOW','OFF'],audio:['ON','OFF']};
function renderSettings(){for(const [k,id] of Object.entries({playerHp:'playerHpSetting',enemyHp:'enemyHpSetting',xpReadout:'xpSetting',shake:'shakeSetting',flash:'flashSetting',particles:'particleSetting',audio:'audioSetting'}))$(id).textContent=save.settings[k]}
...
$('difficultyBtn').onclick=()=>{AUDIO.sfx('ui');nextDifficulty()};
...
for(const [k,id] of Object.entries({playerHp:'playerHpSetting',enemyHp:'enemyHpSetting',xpReadout:'xpSetting',shake:'shakeSetting',flash:'flashSetting',particles:'particleSetting',audio:'audioSetting'}))$(id).onclick=()=>cycleSetting(k);
for(const b of document.querySelectorAll('[data-close]'))b.onclick=()=>closeOverlay(b.dataset.close);
$('codexStatsTab').onclick=()=>{codexTab='stats';renderCodex()};$('codexWeaponsTab').onclick=()=>{codexTab='weapons';renderCodex()};$('codexEnemiesTab').onclick=()=>{codexTab='enemies';renderCodex()};
...
// ---------- INPUT ----------
const keys={},touch={active:false,startX:0,startY:0,x:0,y:0};let gamepadPauseLatch=false;
...
addEventListener('keyup',e=>keys[e.key.toLowerCase()]=false);addEventListener('blur',()=>{if(state?.mode==='run'&&!state.paused&&!state.choosing)pause(true)});
canvas.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse')return;touch.active=true;touch.startX=touch.x=e.clientX;touch.startY=touch.y=e.clientY;canvas.setPointerCapture?.(e.pointerId)});canvas.addEventListener('pointermove',e=>{if(touch.active){touch.x=e.clientX;touch.y=e.clientY}});canvas.addEventListener('pointerup',()=>touch.active=false);canvas.addEventListener('pointercancel',()=>touch.active=false);
...
function critDamage(base){const redline=state.p.hp/state.p.maxHp<=.30?.08:0,drive=state.overdrive>0?.06:0;return Math.random()<Math.min(.68,state.p.crit+redline+drive)?base*1.85:base}
...
function runCredit(amount){const research=1+(save.research.salvage||0)*.04;state.runCredits+=Math.max(1,Math.round(amount*state.p.creditMul*research*state.diff.credits))}
...
function tryEvolutions(){for(const [id,w] of Object.entries(state.weapons)){const m=WEAPON_META[id];if(w.evolved||w.level<6||(state.passives[m.req]||0)<4)continue;w.evolved=true;state.evolutions++;AUDIO.sfx('evolve');addShake(5);flashWrap();message(`EVOLUTION COMPLETE\n${m.evo}`,1450);state.score+=420*state.diff.score;state.newUnlocks.push(`${m.evo} evolution`)}}
function buildOffers(){
  ...
  if(state.level>=9&&Object.keys(state.protocols).length<2)for(const [id,m] of Object.entries(PROTOCOLS))if(!state.protocols[id])offers.push({key:`proto:${id}`,weight:.36,kind:'VOLATILE PROTOCOL',name:m.name,lvl:'ONE-TIME',desc:m.desc,evo:'Permanent risk/reward modifier for this run.',apply:()=>{state.protocols[id]=true;m.apply();AUDIO.sfx('overdrive');message(`PROTOCOL INSTALLED\n${m.name}`,900)}});
  offers.push({key:'repair',weight:state.p.hp<state.p.maxHp*.48?.72:.035,kind:'UTILITY',name:'FIELD REPAIR',lvl:'INSTANT',desc:'Restore 16% maximum hull.',evo:'',apply:()=>state.p.hp=Math.min(state.p.maxHp,state.p.hp+state.p.maxHp*.16)});
...
function selectOffer(o){AUDIO.sfx('select');o.apply();tryEvolutions();hide('levelScreen');state.choosing=false;state.paused=false;state.last=performance.now()}
...
// ---------- RUN START / SPAWNING ----------
function startRun(){
  const frame=SHIPS[save.selected],diff=DIFFICULTIES[save.difficulty]||DIFFICULTIES.STANDARD,now=performance.now();
  const p={x:W/2,y:H/2,r:10,baseSpeed:frame.speed,speed:frame.speed,baseHp:frame.hp,maxHp:frame.hp,hp:frame.hp,frame:save.selected,baseArmor:frame.armor,moduleArmor:0,baseDamageMul:frame.damage,damageMul:frame.damage,baseCooldownMul:1/frame.fire,cooldownMul:1/frame.fire,projectileMul:1,baseCrit:frame.crit,crit:frame.crit,magnet:78,baseMagnet:78,convertChance:0,allyCap:0,areaMul:1,specialMul:1,hitFlash:0,iFrames:0,incomingMul:1,scoreMul:1,creditMul:1,cacheLuck:0,grazeRadius:13};
  applyResearchToPlayer(p);
  state={mode:'run',diff,difficulty:save.difficulty,time:0,last:now,paused:false,choosing:false,gameOver:false,victory:false,revived:false,runCredits:0,kills:0,score:0,spawnT:0,eliteT:30,bossStage:0,level:1,xp:0,xpNeed:8,rerolls:1+(save.research.reroll||0),skips:1,currentOffers:[],runCounted:false,accountedKills:0,accountedCredits:0,threat:1,chain:0,chainBest:0,chainTimer:0,nextBounty:65,bounty:null,nextAnomaly:115,redline:false,overdrive:0,grazes:0,cachesOpened:0,conversions:0,evolutions:0,damageDealt:0,damageTaken:0,bossesKilled:0,weaponDamage:{},noHit:true,newUnlocks:[],protocols:{},p,passives:{},weapons:{},enemies:[],bullets:[],enemyBullets:[],orbs:[],caches:[],mines:[],allies:[],particles:[],arcs:[],beams:[],shake:0,stars:Array.from({length:110},()=>({x:Math.random()*W,y:Math.random()*H,z:1+Math.random()*2}))};
  if(frame.startPassive){state.passives[frame.startPassive]=1;PASSIVES[frame.startPassive].apply(1)}
  addWeapon(frame.weapon);AUDIO.ensure();AUDIO.resetMusic();AUDIO.sfx('start');hideAll();hud.classList.remove('hidden');message(`ORBIT//04\n${frame.name} · ${diff.name}`,950);checkMetaAchievements();
}
...
function spawnEnemy(forceType=null,elite=false,at=null){
  ...
  discoverEnemy(type);const b=enemyBase(type),hpScale=(1+t/250)*state.diff.hp,damageScale=(1+Math.min(.80,t/820))*state.diff.damage,speedScale=(1+Math.min(.50,t/720))*state.diff.speed;
...
function spawnBoss(stage){AUDIO.sfx('boss');discoverEnemy('boss');const b=BOSSES[stage],pos=edgePos(60),hp=b.hp*state.diff.hp*(1+state.time/900);state.enemies.push({...pos,type:'boss',hp,maxHp:hp,speed:b.speed*state.diff.speed,r:29+stage*2,damage:b.damage*state.diff.damage,xp:55+stage*18,color:b.color,elite:true,boss:true,bounty:false,bossStage:stage,bossName:b.name,dead:false,hit:0,shootT:.55,chargeT:2,pattern:0,spawnT:4});addShake(8);message(`WARNING\n${b.name} SIGNATURE`,1500)}
...
function triggerAnomaly(){const roll=Math.random();if(roll<.5){message('ANOMALY\nDENSE HOSTILE PACKET',850);for(let i=0;i<8+state.threat;i++)spawnEnemy();spawnEnemy(null,true)}else{message('ANOMALY\nSALVAGE BURST',850);for(let i=0;i<10;i++){const a=Math.random()*Math.PI*2,r=80+Math.random()*100;state.orbs.push({x:clamp(state.p.x+Math.cos(a)*r,20,W-20),y:clamp(state.p.y+Math.sin(a)*r,20,H-20),r:5,val:2,dead:false})}}state.nextAnomaly+=105+Math.random()*35}
function updatePressure(){const threat=1+Math.floor(state.time/55);if(threat>state.threat){state.threat=threat;AUDIO.sfx('danger');message(`THREAT ${String(threat).padStart(2,'0')}\nHOSTILES ESCALATING`,800)}if(state.time>=state.nextBounty){startBounty();state.nextBounty+=72}if(state.time>=state.nextAnomaly)triggerAnomaly();if(state.bounty&&state.time>state.bounty.until){const e=state.bounty.enemy;if(e&&!e.dead){e.bounty=false;e.speed*=1.35;e.damage*=1.35;e.color='#ff3f59';AUDIO.sfx('danger');message('SIGNAL LOST\nTARGET ENRAGED',750)}state.bounty=null}}
function spawnWave(dt){state.spawnT-=dt;const base=Math.max(.075,.55-state.time*.00065),interval=base/state.diff.spawn;if(state.spawnT<=0){state.spawnT=interval;spawnEnemy();if(state.time>75&&Math.random()<Math.min(.78,state.time/600))spawnEnemy();if(state.time>330&&Math.random()<.28)spawnEnemy()}if(state.time>=state.eliteT){spawnEnemy(null,true);state.eliteT+=30+Math.random()*12}const bossAlive=state.enemies.some(e=>e.boss&&!e.dead);if(state.bossStage===0&&state.time>=240&&!bossAlive){state.bossStage=1;spawnBoss(1)}else if(state.bossStage===1&&state.time>=480&&!bossAlive){state.bossStage=2;spawnBoss(2)}else if(state.bossStage===2&&state.time>=RUN_TARGET&&!bossAlive){state.bossStage=3;spawnBoss(3)}}
...
function weaponDrone(w,dt){w.timer-=dt;if(w.timer>0)return;const target=nearest(state.p);if(!target)return;w.timer=.73*(w.level>=4?.84:1)*(w.evolved?.68:1)*state.p.cooldownMul;const dmg=10*(1+(w.level-1)*.18)*(w.evolved?1.48:1)*state.p.damageMul;AUDIO.sfx('drone');for(const d of dronePositions(w))fireProjectile(d.x,d.y,target,395*state.p.projectileMul,critDamage(dmg),{color:'#8dffd6',weaponId:'drone'})}
...
function weaponRail(w,dt){w.timer-=dt;if(w.timer>0)return;const target=nearest(state.p);if(!target)return;const l=w.level,e=w.evolved,n=(l>=5?2:1)+(e?1:0),dmg=57*(1+(l-1)*.23)*(e?1.48:1)*state.p.damageMul;w.timer=1.36*(l>=4?.84:1)*(e?.72:1)*state.p.cooldownMul;AUDIO.sfx('rail');for(let i=0;i<n;i++)fireProjectile(state.p.x,state.p.y,target,625*state.p.projectileMul,critDamage(dmg),{spread:(i-(n-1)/2)*.075,pierce:2+(l>=2?1:0)+(l>=6?2:0)+(e?3:0),r:3,color:'#fff',weaponId:'rail'})}
...
function weaponBeam(w,dt){w.timer-=dt;if(w.timer>0)return;const target=nearest(state.p);if(!target)return;const l=w.level,e=w.evolved,n=(l>=5?2:1)+(e?1:0),range=500,dmg=38*(1+(l-1)*.20)*(e?1.42:1)*state.p.damageMul;w.timer=.92*(l>=4?.84:1)*(e?.70:1)*state.p.cooldownMul/(state.p.specialMul||1);AUDIO.sfx('beam');const base=Math.atan2(target.y-state.p.y,target.x-state.p.x);for(let q=0;q<n;q++){const a=base+(q-(n-1)/2)*.11,ex=state.p.x+Math.cos(a)*range,ey=state.p.y+Math.sin(a)*range;state.beams.push({x1:state.p.x,y1:state.p.y,x2:ex,y2:ey,life:.1,color:e?'#fff':'#78caff'});const hits=[];for(const x of state.enemies){if(x.dead)continue;const vx=x.x-state.p.x,vy=x.y-state.p.y,along=vx*Math.cos(a)+vy*Math.sin(a),perp=Math.abs(-vx*Math.sin(a)+vy*Math.cos(a));if(along>0&&along<range&&perp<x.r+6)hits.push({e:x,d:along})}hits.sort((A,B)=>A.d-B.d);const cap=2+(l>=2?1:0)+(e?3:0);for(const h of hits.slice(0,cap))damageEnemy(h.e,critDamage(dmg),false,'beam')}}
function updateWeapons(dt){const drive=state.overdrive>0?1.30:1;const map={pulse:weaponPulse,missile:weaponMissile,drone:weaponDrone,arc:weaponArc,rail:weaponRail,nova:weaponNova,mine:weaponMine,beam:weaponBeam};for(const [id,w] of Object.entries(state.weapons))map[id]?.(w,dt*drive)}

function convertEnemy(e){if(e.dead)return;AUDIO.sfx('convert');if(state.bounty?.enemy===e)state.bounty=null;e.dead=true;state.conversions++;save.stats.conversions++;state.allies.push({x:e.x,y:e.y,r:8,life:14+(state.passives.iff||0)*3,fireT:.2+Math.random()*.4,angle:Math.random()*Math.PI*2});particle(e.x,e.y,'#8dffd6',10);message('IFF OVERRIDE\nALLY ACQUIRED',580);state.score+=30*state.diff.score;unlockAchievement('iff_friend')}
function damageEnemy(e,damage,conversionEligible=true,weaponId='unknown'){if(e.dead)return;if(conversionEligible&&!e.boss&&!e.elite&&state.p.convertChance>0&&state.allies.length<state.p.allyCap&&Math.random()<state.p.convertChance){convertEnemy(e);return}const actual=Math.max(0,Math.min(e.hp,damage));e.hp-=damage;e.hit=.07;state.damageDealt+=actual;state.weaponDamage[weaponId]=(state.weaponDamage[weaponId]||0)+actual;particle(e.x,e.y,'#d8fbff',2);if(e.hp<=0)killEnemy(e)}
...
function autoUpgrade(times=1){let done=0;for(let i=0;i<times;i++){const pool=eligibleAutoUpgrades();if(!pool.length)break;choice(pool)();done++}tryEvolutions();return done}
function dropCache(e){if(!e.elite)return;const guaranteed=e.boss||e.bounty;if(!guaranteed&&Math.random()>.42)return;let roll=Math.max(0,Math.random()-state.p.cacheLuck),rarity='CACHE';if(e.boss){if(roll<.12)rarity='OMEGA';else if(roll<.46)rarity='RARE'}else{if(roll<.025)rarity='OMEGA';else if(roll<.17)rarity='RARE'}state.caches.push({x:e.x,y:e.y,r:7,rarity,life:28,dead:false})}
function openCache(c){c.dead=true;state.cachesOpened++;let upgrades=1,credits=5,rerolls=0,heal=0;if(c.rarity==='RARE'){upgrades=2;credits=14;rerolls=1}else if(c.rarity==='OMEGA'){upgrades=3;credits=30;rerolls=1;heal=.12;unlockAchievement('omega')}const gained=autoUpgrade(upgrades);runCredit(credits);state.rerolls=Math.min(6,state.rerolls+rerolls);if(heal)state.p.hp=Math.min(state.p.maxHp,state.p.hp+state.p.maxHp*heal);state.score+=(c.rarity==='OMEGA'?850:c.rarity==='RARE'?360:140)*state.diff.score;AUDIO.sfx(c.rarity==='OMEGA'?'omega':'cache');addShake(c.rarity==='OMEGA'?7:3);flashWrap();message(`${c.rarity} DATA\n${gained} UPGRADE${gained===1?'':'S'} · +${Math.round(credits*state.p.creditMul)} CR${rerolls?' · +1 REROLL':''}`,1050)}
...
function killEnemy(e){
  if(e.dead)return;e.dead=true;state.kills++;if(state.kills===1)unlockAchievement('first_blood');splitEnemy(e);AUDIO.sfx(e.elite?'elite':'kill');
  state.chain=state.chainTimer>0?state.chain+1:1;state.chainTimer=2.15;state.chainBest=Math.max(state.chainBest,state.chain);if(state.chain===10||state.chain===25||state.chain%50===0)AUDIO.sfx('streak');if(state.chain===25||state.chain===60||state.chain%100===0){state.overdrive=Math.max(state.overdrive,state.chain>=100?8:state.chain>=60?6:4);AUDIO.sfx('overdrive');message(`OVERDRIVE\n${state.overdrive.toFixed(0)} SEC`,550)}if(state.chain>=100)unlockAchievement('chain_100');
  const chainMul=1+Math.min(1.6,Math.floor(state.chain/5)*.10),redlineMul=state.p.hp/state.p.maxHp<=.30?1.25:1;state.score+=(10+e.xp*8+(e.elite?70:0)+(e.boss?500:0))*chainMul*redlineMul*state.p.scoreMul*state.diff.score;
  const bountyWin=state.bounty?.enemy===e&&state.time<=state.bounty.until;if(bountyWin){runCredit(14);state.rerolls=Math.min(6,state.rerolls+1);state.score+=360*state.diff.score;AUDIO.sfx('bounty');message('SIGNAL CAPTURED\n+SALVAGE · +1 REROLL',780);state.bounty=null;unlockAchievement('signal_capture')}
  dropCache(e);state.orbs.push({x:e.x,y:e.y,r:e.boss?8:4,val:e.xp*(bountyWin?2:1),dead:false});particle(e.x,e.y,e.boss?'#ff8994':e.color,e.boss?32:e.elite?15:7);
  if(e.elite){runCredit(e.boss?38:7);if(e.boss){state.bossesKilled++;unlockAchievement('first_boss')}}
  if(e.boss&&e.bossStage===3)finishRun(true)
}
function enemyShoot(e){AUDIO.sfx('enemyShot');const a=Math.atan2(state.p.y-e.y,state.p.x-e.x);if(e.boss){e.pattern=(e.pattern+1)%3;const stage=e.bossStage;if(e.pattern===0){const n=8+stage*3;for(let i=0;i<n;i++){const aa=(i/n)*Math.PI*2+state.time*.22;state.enemyBullets.push({x:e.x,y:e.y,vx:Math.cos(aa)*(155+stage*12),vy:Math.sin(aa)*(155+stage*12),r:4,damage:(11+stage*2)*state.diff.damage,life:4,grazed:false})}}else if(e.pattern===1){for(let i=-1;i<=1;i++){const aa=a+i*.16;state.enemyBullets.push({x:e.x,y:e.y,vx:Math.cos(aa)*(205+stage*15),vy:Math.sin(aa)*(205+stage*15),r:4,damage:(13+stage*2)*state.diff.damage,life:4,grazed:false})}}else if(stage>=2){for(let i=0;i<6;i++){const aa=(i/6)*Math.PI*2-state.time*.15;state.enemyBullets.push({x:e.x,y:e.y,vx:Math.cos(aa)*205,vy:Math.sin(aa)*205,r:3,damage:(10+stage)*state.diff.damage,life:4,grazed:false})}}return}
  const speed=e.type==='sniper'?300:195,damage=(e.type==='sniper'?15:10)*state.diff.damage;state.enemyBullets.push({x:e.x,y:e.y,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,r:e.type==='sniper'?3:3,damage,life:4,grazed:false})
}
function allyShoot(a){const target=nearest(a);if(!target)return;fireProjectile(a.x,a.y,target,400,state.p.damageMul*10,{color:'#8dffd6',weaponId:'ally'})}
function hitPlayer(dmg){const p=state.p;if(p.iFrames>0)return;AUDIO.sfx('hit');const mit=clamp(p.baseArmor+p.moduleArmor,0,.45),actual=dmg*(1-mit)*p.incomingMul;p.hp-=actual;p.hitFlash=.16;p.iFrames=.36;state.damageTaken+=actual;state.noHit=false;state.chain=0;state.chainTimer=0;addShake(7);if(p.hp<=0)finishRun(false)}

// ---------- UPDATE ----------
function update(dt){
  if(!state||state.mode!=='run'||state.paused)return;
  state.time+=dt;state.shake=Math.max(0,state.shake-dt*18);AUDIO.music(state.time,state.enemies.some(e=>e.boss&&!e.dead));const p=state.p;state.overdrive=Math.max(0,state.overdrive-dt);p.hitFlash=Math.max(0,p.hitFlash-dt);p.iFrames=Math.max(0,p.iFrames-dt);state.chainTimer=Math.max(0,state.chainTimer-dt);if(state.chainTimer<=0)state.chain=0;
  const redline=p.hp/p.maxHp<=.30;if(redline&&!state.redline){state.redline=true;AUDIO.sfx('danger');message('REDLINE\nCRIT +8% · SCORE +25%',700)}else if(!redline)state.redline=false;
  const chainMul=1+Math.min(1.6,Math.floor(state.chain/5)*.10);state.score+=dt*(1+state.level*.10)*chainMul*(redline?1.25:1)*state.p.scoreMul*state.diff.score;updatePressure();
  const input=moveInput();let dx=input.dx,dy=input.dy;if(dx||dy){const l=Math.hypot(dx,dy);const driveSpeed=state.overdrive>0?1.12:1;p.x+=dx/l*p.speed*driveSpeed*dt;p.y+=dy/l*p.speed*driveSpeed*dt}p.x=clamp(p.x,14,W-14);p.y=clamp(p.y,14,H-14);
  ...
  for(const e of state.enemies){
    if(e.dead)continue;e.hit=Math.max(0,e.hit-dt);const a=Math.atan2(p.y-e.y,p.x-e.x);let speed=e.speed;
    ...
    if(e.type==='gunner'||e.type==='sniper'||e.boss){e.shootT-=dt;if(e.shootT<=0){enemyShoot(e);e.shootT=e.boss?Math.max(.42,.76-e.bossStage*.08):(e.type==='sniper'?1.9:1.45)}}
    if(e.boss&&e.bossStage===3){e.spawnT-=dt;if(e.spawnT<=0){e.spawnT=4.2;for(let i=0;i<3;i++)spawnEnemy(i===0?'gunner':'scout',false,{x:e.x+(Math.random()-.5)*50,y:e.y+(Math.random()-.5)*50})}}
  ...
  for(const m of state.mines){m.life-=dt;let target=null;for(const e of state.enemies){if(!e.dead&&dist2(m,e)<45*45){target=e;break}}if(target){m.life=0;AUDIO.sfx('mine');addShake(3);state.arcs.push({circle:true,x1:m.x,y1:m.y,r:m.blast,life:.12,color:'#bd9cff'});for(const e of state.enemies)if(!e.dead&&dist2(m,e)<m.blast*m.blast)damageEnemy(e,critDamage(m.damage),false,'mine')}}
  for(const b of state.enemyBullets){b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;const d2=dist2(b,p);if(d2<(b.r+p.r)*(b.r+p.r)){b.life=0;hitPlayer(b.damage)}else if(!b.grazed&&d2<(b.r+p.r+p.grazeRadius)*(b.r+p.r+p.grazeRadius)){b.grazed=true;state.grazes++;state.score+=14*state.p.scoreMul*state.diff.score;state.chainTimer=Math.max(state.chainTimer,.5);AUDIO.sfx('graze');if(state.grazes>=25)unlockAchievement('graze_25')}}
  ...
  for(const a of state.allies){a.life-=dt;const target=nearest(a);if(target){const ang=Math.atan2(target.y-a.y,target.x-a.x);a.x+=Math.cos(ang)*48*dt;a.y+=Math.sin(ang)*48*dt}a.fireT-=dt;if(a.fireT<=0){a.fireT=.72;allyShoot(a)}}
...
// ---------- PAUSE / RUN END ----------
function pause(on=true){if(!state||state.choosing||state.gameOver)return;AUDIO.sfx(on?'pause':'resume');state.paused=on;if(on)show('pauseScreen');else hide('pauseScreen');if(!on)state.last=performance.now()}
...
function draw(){
  ...
  for(const a of state.allies)drawShip(a.x,a.y,'#8dffd6',.78,-Math.PI/2,'striker');
  ...
  const p=state.p;drawShip(p.x,p.y,p.hitFlash?'#ff9aa3':'#d9f5ff',1,-Math.PI/2,p.frame);const php=save.settings.playerHp;if(php==='BAR'||php==='BOTH'){const bw=34;px(p.x-bw/2,p.y-20,bw,3,'#15232a');px(p.x-bw/2,p.y-20,bw*clamp(p.hp/p.maxHp,0,1),3,p.hp/p.maxHp<=.30?'#ff6f7d':'#8dffd6')}
  ...
  const chainMul=1+Math.min(1.6,Math.floor(state.chain/5)*.10),bounty=state.bounty?` | SIGNAL ${Math.max(0,state.bounty.until-state.time).toFixed(1)}s`:'';hudRight.textContent=`${state.difficulty} | ${fmtTime(state.time)} | CHAIN ${state.chain} x${chainMul.toFixed(1)}${state.overdrive>0?` | DRIVE ${state.overdrive.toFixed(1)}s`:''} | ${Math.floor(state.score)} | +${state.runCredits} CR${bounty}`;
  ...
  const boss=state.enemies.find(e=>e.boss&&!e.dead);if(boss){bossBar.classList.remove('hidden');bossLabel.textContent=boss.bossName;bossFill.style.width=`${clamp(boss.hp/boss.maxHp,0,1)*100}%`}else bossBar.classList.add('hidden');
}
