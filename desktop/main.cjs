...
async function configureCaptureScene(win, preset) {
  return win.webContents.executeJavaScript(`(() => {
    ...
    updateDirectiveHud();
    if(preset.slug==='02-pulsar-signal-rush')toast('NEW HOSTILE · PALE WARDEN',ENEMY_META.warden.quick,'intel');
...
const inspect=(id,allowVertical=true)=>{const overlay=$(id),panel=overlay.querySelector('.panel'),wrapRect=$('wrap').getBoundingClientRect(),rect=panel.getBoundingClientRect(),buttons=[...panel.querySelectorAll('button:not(.hidden)')].filter(button=>!button.disabled&&!button.closest('.hidden')&&!button.closest('details:not([open])')),buttonRects=buttons.map(button=>button.getBoundingClientRect()),horizontalOverflow=panel.scrollWidth>panel.clientWidth+1,verticalOverflow=panel.scrollHeight>panel.clientHeight+1,withinViewport=rect.left>=wrapRect.left-1&&rect.right<=wrapRect.right+1&&rect.top>=wrapRect.top-1&&rect.bottom<=wrapRect.bottom+1,usableButtons=buttonRects.every(button=>button.width>=21.5&&button.height>=21.5),minimumButton=buttonRects.length?{width:Math.round(Math.min(...buttonRects.map(button=>button.width))),height:Math.round(Math.min(...buttonRects.map(button=>button.height)))}:null,smallButtons=buttonRects.map((button,index)=>({id:buttons[index].id||buttons[index].className,width:Math.round(button.width),height:Math.round(button.height)})).filter(button=>button.width<22||button.height<22);return{id,horizontalOverflow,verticalOverflow,allowVertical,withinViewport,usableButtons,minimumButton,smallButtons,buttonCount:buttons.length,client:{width:panel.clientWidth,height:panel.clientHeight},scroll:{width:panel.scrollWidth,height:panel.scrollHeight}}};
      const screens=[];
      toMenu();renderMenu();screens.push(inspect('titleScreen',true));
      const title=$('titleScreen'),more=title.querySelector('.secondaryActions'),summary=more.querySelector('summary');
      more.open=false;const closedControls=gamepadUiButtons(title),canOpen=closedControls.includes(summary),hiddenExcluded=!closedControls.includes($('briefingMenuBtn'));
      summary.click();const expandedReachable=gamepadUiButtons(title).includes($('briefingMenuBtn'));more.open=false;
      const tabEvent=new KeyboardEvent('keydown',{key:'Tab',bubbles:true,cancelable:true});summary.dispatchEvent(tabEvent);
      const nativeTab=!tabEvent.defaultPrevented;resetInputState();
      const card=title.querySelector('.shipCard');card.focus();card.click();const focusRetained=document.activeElement?.dataset.frameId===save.selected;
      const lockedId=Object.keys(SHIPS).find(id=>!save.unlocked.includes(id)),creditsBefore=save.credits,selectedBefore=save.selected;
      if(lockedId)previewFrame(lockedId);
      const previewSafe=save.credits===creditsBefore&&save.selected===selectedBefore;
      screens.push({...inspect('titleScreen',true),id:'framePreview'});
      previewFrameId='';renderMenu();
      if(!canOpen||!hiddenExcluded||!expandedReachable||!nativeTab||!focusRetained||!previewSafe)throw new Error('Hangar navigation regression: '+JSON.stringify({canOpen,hiddenExcluded,expandedReachable,nativeTab,focusRetained,previewSafe}));
      startRun();toastStack.innerHTML='';save.briefedEnemies=[];state.enemyIntelQueue=[];state.enemyIntelShown=0;state.nextEnemyIntel=0;state.time=20;spawnEnemy('warden',false,{x:state.p.x+220,y:state.p.y});updateDirectiveHud();updateEnemyIntel();const directiveRect=$('directiveTracker').getBoundingClientRect(),latestToast=toastStack.children[toastStack.children.length-1],toastRect=latestToast?.getBoundingClientRect(),hudToastSafe=!!toastRect&&(toastRect.bottom<=directiveRect.top||toastRect.top>=directiveRect.bottom||toastRect.right<=directiveRect.left||toastRect.left>=directiveRect.right);state.time=195;state.bossStage=0;updateHudVisuals();const bossApproach={visible:!bossBar.classList.contains('hidden'),incoming:bossBar.classList.contains('incoming'),label:bossLabel.textContent,progress:Math.round(parseFloat(bossFill.style.width)||0)};pause(true);screens.push(inspect('pauseScreen',true));
      const pauseTab=new KeyboardEvent('keydown',{key:'Tab',bubbles:true,cancelable:true});$('resumeBtn').dispatchEvent(pauseTab);resetInputState();
      ...
      if(!bossApproach.visible||!bossApproach.incoming||bossApproach.label!=='SIGNATURE APPROACH · 15.0 SEC'||bossApproach.progress!==25)visible.push({id:'bossApproach',reason:'boss countdown is missing or inaccurate',state:bossApproach});
      return{viewport:{width:innerWidth,height:innerHeight},hudToastSafe,bossApproach,screens,failures:visible};
    })()`);
    report.push(result);
  }
