...
async function runLayoutSmoke(win) {
  ...
  for (const size of sizes) {
    ...
    const result = await win.webContents.executeJavaScript(`(() => {
      ...
      startRun();toastStack.innerHTML='';save.briefedEnemies=[];state.enemyIntelQueue=[];state.enemyIntelShown=0;state.nextEnemyIntel=0;state.time=20;spawnEnemy('warden',false,{x:state.p.x+220,y:state.p.y});updateDirectiveHud();updateEnemyIntel();const directiveRect=$('directiveTracker').getBoundingClientRect(),latestToast=toastStack.children[toastStack.children.length-1],toastRect=latestToast?.getBoundingClientRect(),hudToastSafe=!!toastRect&&(toastRect.bottom<=directiveRect.top||toastRect.top>=directiveRect.bottom||toastRect.right<=directiveRect.left||toastRect.left>=directiveRect.right);pause(true);screens.push(inspect('pauseScreen',true));
      ...
      return{viewport:{width:innerWidth,height:innerHeight},hudToastSafe,screens,failures:visible};
    })()`);
    report.push(result);
  }
