...
export class OrbitThreeVisualEngine{
  ...
  syncTransientFx(state,quality,glow,settings){
    ...
    const telegraphs=settings.telegraphs==='OFF'?[]:state.enemies.filter(enemy=>!enemy.dead&&(enemy.type==='gunner'||enemy.type==='sniper'||enemy.type==='weaver'||enemy.boss)&&enemy.shootT>0&&enemy.shootT<.42).map(enemy=>({x1:enemy.x,y1:enemy.y,x2:state.p.x,y2:state.p.y,width:enemy.boss?1.8:1.1,alpha:clamp((.42-enemy.shootT)/.42)*.66,color:enemy.boss?'#c83e53':enemy.type==='sniper'?'#c69b59':enemy.type==='weaver'?'#7f89ff':'#ad5d68'}));
