...
export class OrbitThreeVisualEngine{
  constructor(canvas,{width=960,height=540,scale=1.5}={}){
    ...
    this.loader=new THREE.TextureLoader();this.glowTexture=radialTexture();this.engineTexture=engineTexture();this.projectileTexture=projectileTexture();this.burstTexture=burstTexture();this.textures={
  ...
  sync(gameState,settings={}){
    ...
    this.renderer.toneMappingExposure=quality==='ULTRA'?1.18:quality==='LOW'?1.03:1.12;this.renderer.render(this.scene,this.camera);
  }
  ...
  syncEnemies(state,settings,glow){
    const enemies=state.enemies.filter(enemy=>!enemy.dead),clarity=settings.effectClarity||'HIGH',clarityBoost=clarity==='HIGH'?1.42:clarity==='BALANCED'?1.18:1;
