...
export class OrbitThreeVisualEngine{
  constructor(canvas,{width=960,height=540,scale=1.5}={}){
    ...
    this.loader=new THREE.TextureLoader();this.glowTexture=radialTexture();this.shadowTexture=shadowTexture();this.engineTexture=engineTexture();this.projectileTexture=projectileTexture();this.burstTexture=burstTexture();this.textures={
      player:this.load('assets/visuals/player-last-ark-v1.png'),muzzle:this.load('assets/visuals/weapon-muzzle-premium-v1.png'),projectileSource:this.load('assets/visuals/weapon-projectile-premium-v1.png'),projectile:this.projectileTexture
    };
  ...
  syncPlayer(state,settings,glow){
    ...
    this.playerTrail.visible=glow;this.placeSprite(this.playerTrail,trailX,trailY,trailLength*1.20,15+m.thrust*6,m.angle,2,(.13+.17*m.thrust+.14*dash)*enginePulse,combatPalette.friendly);
    this.playerEngineLeft.visible=this.playerEngineRight.visible=true;for(const [engine,side] of [[this.playerEngineLeft,-1],[this.playerEngineRight,1]]){const output=1-side*m.engineBias,offset=side*(5.5+Math.abs(m.bank)*3),length=trailLength*output,ex=x+backX*(25+length*.48)+sideX*offset,ey=y+backY*(25+length*.48)+sideY*offset,flutter=side<0?enginePulse:1.02-enginePulse*.10;this.placeSprite(engine,ex,ey,length,6+m.thrust*3,m.angle,3,(.40+.50*m.thrust+.20*dash)*flutter*(.92+output*.08),side<0?combatPalette.friendly:combatPalette.friendlySoft)}
