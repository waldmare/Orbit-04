...
const projectileHeading=projectile=>Math.atan2(Number.isFinite(projectile?.vy)?projectile.vy:0,Number.isFinite(projectile?.vx)?projectile.vx:1);
...
const enemyTexture={
  ...
  stalker:'assets/visuals/enemy-hunter-v2.png',
  weaver:'assets/visuals/enemy-witness-v1.png',
  warden:'assets/visuals/enemy-ossuary-v1.png',
  reaper:'assets/visuals/enemy-hunter-v2.png',
  harrower:'assets/visuals/enemy-witness-v1.png',
  moth:'assets/visuals/enemy-hunter-v2.png',
  anchor:'assets/visuals/enemy-ossuary-v1.png',
  boss:'assets/visuals/boss-conquest-leviathan-v1.png'
};

const combatPalette=Object.freeze({friendly:'#35efff',friendlySoft:'#9fffe8',heavy:'#ffb347',hostile:'#ff2d82',danger:'#ff435f',anomaly:'#a879ff',support:'#72e89b',void:'#07030a'});
const enemyAccent={scout:'#ff4d86',charger:'#ff684c',tank:'#d79d49',gunner:'#ff2d82',splitter:'#d64b79',sniper:'#a879ff',stalker:'#ed48c9',weaver:'#7f89ff',warden:'#55d1ae',reaper:'#bdf34b',harrower:'#ff8b3d',moth:'#e75cff',anchor:'#5f7cff',boss:'#ff214f'};
const enemyBodyTint={scout:'#e889a4',charger:'#e99172',tank:'#c5a66c',gunner:'#e378a0',splitter:'#c77a98',sniper:'#b79add',stalker:'#dc78ca',weaver:'#9ca3e8',warden:'#7fc6b1',reaper:'#a7c966',harrower:'#d99563',moth:'#d883e3',anchor:'#8497e5',boss:'#f07891'};
const projectileAccent={pulse:combatPalette.friendly,missile:combatPalette.heavy,drone:'#73fbd3','hunter-wing':combatPalette.heavy,rail:'#fff5dc',flak:combatPalette.heavy,fragmentation:'#ffcf63',ally:'#73fbd3',unknown:combatPalette.friendlySoft};
const markedEnemyTypes=new Set(['charger','tank','gunner','splitter','sniper','stalker','weaver','warden','reaper','harrower','moth','anchor']);
const rangedThreatTypes=new Set(['gunner','sniper','weaver','harrower','anchor']);
const dashThreatTypes=new Set(['charger','stalker','reaper']);
const dashWarningTimer=enemy=>enemy.type==='charger'?enemy.chargeT:enemy.abilityT;
const isDashIntent=enemy=>dashThreatTypes.has(enemy.type)&&(enemy.burst>0||enemy.burst<=0&&dashWarningTimer(enemy)>0&&dashWarningTimer(enemy)<.58);
const isRangedIntent=(enemy,window=.65)=>(enemy.boss||rangedThreatTypes.has(enemy.type))&&enemy.shootT>0&&enemy.shootT<window;
...
function shadowTexture(){
  const canvas=document.createElement('canvas');canvas.width=192;canvas.height=96;const context=canvas.getContext('2d'),gradient=context.createRadialGradient(96,48,3,96,48,88);
  gradient.addColorStop(0,'rgba(255,255,255,.92)');gradient.addColorStop(.28,'rgba(255,255,255,.58)');gradient.addColorStop(.68,'rgba(255,255,255,.15)');gradient.addColorStop(1,'rgba(255,255,255,0)');context.fillStyle=gradient;context.fillRect(0,0,192,96);const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.minFilter=texture.magFilter=THREE.LinearFilter;texture.generateMipmaps=false;texture.name='depth-contact-shadow';return texture;
}
...
function projectileTexture(){
  const canvas=document.createElement('canvas');canvas.width=320;canvas.height=96;const context=canvas.getContext('2d');context.translate(0,48);context.globalCompositeOperation='lighter';
  const aura=context.createLinearGradient(0,0,320,0);aura.addColorStop(0,'rgba(255,255,255,0)');aura.addColorStop(.30,'rgba(255,255,255,.08)');aura.addColorStop(.76,'rgba(255,255,255,.38)');aura.addColorStop(1,'rgba(255,255,255,.96)');context.fillStyle=aura;context.beginPath();context.moveTo(4,0);context.quadraticCurveTo(205,-31,314,0);context.quadraticCurveTo(205,31,4,0);context.fill();
  const core=context.createLinearGradient(18,0,318,0);core.addColorStop(0,'rgba(255,255,255,0)');core.addColorStop(.48,'rgba(255,255,255,.42)');core.addColorStop(.86,'rgba(255,255,255,.96)');core.addColorStop(1,'#fff');context.fillStyle=core;context.beginPath();context.moveTo(18,0);context.lineTo(274,-7);context.lineTo(318,0);context.lineTo(274,7);context.closePath();context.fill();
  context.strokeStyle='rgba(255,255,255,.82)';context.lineWidth=2;context.beginPath();context.moveTo(64,0);context.lineTo(303,0);context.stroke();
  const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.minFilter=texture.magFilter=THREE.LinearFilter;texture.generateMipmaps=false;texture.name='axis-aligned-projectile';return texture;
}

function burstTexture(){
  const canvas=document.createElement('canvas');canvas.width=canvas.height=256;const context=canvas.getContext('2d');context.translate(128,128);context.globalCompositeOperation='lighter';
  for(let ray=0;ray<16;ray++){const angle=ray/16*TAU,length=ray%2?72:106,width=ray%2?2:4;context.strokeStyle=ray%3?'rgba(255,255,255,.38)':'rgba(255,255,255,.72)';context.lineWidth=width;context.beginPath();context.moveTo(Math.cos(angle)*15,Math.sin(angle)*15);context.lineTo(Math.cos(angle)*length,Math.sin(angle)*length);context.stroke()}
  const glow=context.createRadialGradient(0,0,0,0,0,104);glow.addColorStop(0,'rgba(255,255,255,1)');glow.addColorStop(.10,'rgba(255,255,255,.94)');glow.addColorStop(.30,'rgba(255,255,255,.44)');glow.addColorStop(.62,'rgba(255,255,255,.11)');glow.addColorStop(1,'rgba(255,255,255,0)');context.fillStyle=glow;context.beginPath();context.arc(0,0,104,0,TAU);context.fill();
...
function pickupIconTexture(kind){
  ...
  if(kind==='orb'){
    ...
    context.beginPath();context.arc(0,0,31,0,TAU);context.fill();context.stroke();context.beginPath();context.moveTo(0,-20);context.lineTo(0,3);context.lineTo(17,14);context.stroke();context.beginPath();context.arc(0,0,5,0,TAU);context.fillStyle='#fff';context.fill();
  }else if(kind==='relic'){
...
function enemyRoleTexture(kind){
  ...
  if(kind==='charger'){
    ...
    for(const y of [-15,0,15]){context.beginPath();context.moveTo(-22,y-7);context.lineTo(0,y+5);context.lineTo(22,y-7);context.stroke()}
  }else if(kind==='weaver'){
    context.beginPath();context.moveTo(-24,-22);context.lineTo(24,22);context.moveTo(24,-22);context.lineTo(-24,22);context.stroke();context.beginPath();context.arc(0,0,7,0,TAU);context.fillStyle='#fff';context.fill();
  }else if(kind==='warden'){
    context.beginPath();context.moveTo(0,-27);context.lineTo(24,-13);context.lineTo(19,16);context.lineTo(0,29);context.lineTo(-19,16);context.lineTo(-24,-13);context.closePath();context.fill();context.stroke();context.beginPath();context.arc(0,0,9,0,TAU);context.stroke();
  }else if(kind==='reaper'){
    context.beginPath();context.arc(0,0,25,-Math.PI*.70,Math.PI*.70);context.stroke();context.beginPath();context.moveTo(-5,-24);context.lineTo(24,0);context.lineTo(-5,24);context.stroke();
  }else if(kind==='harrower'){
    for(const x of [-18,0,18]){context.beginPath();context.moveTo(x,25);context.lineTo(x*.42,-26);context.stroke()}context.beginPath();context.moveTo(-27,12);context.lineTo(0,-7);context.lineTo(27,12);context.stroke();
  }else if(kind==='moth'){
    context.beginPath();context.moveTo(0,-24);context.lineTo(25,-12);context.lineTo(11,6);context.lineTo(24,22);context.lineTo(0,10);context.lineTo(-24,22);context.lineTo(-11,6);context.lineTo(-25,-12);context.closePath();context.fill();context.stroke();
  }else if(kind==='anchor'){
    context.beginPath();context.arc(0,0,20,0,TAU);context.stroke();context.beginPath();context.moveTo(-28,0);context.lineTo(28,0);context.moveTo(0,-28);context.lineTo(0,28);context.stroke();context.beginPath();context.arc(0,0,5,0,TAU);context.fillStyle='#fff';context.fill();
  }else{
...
export class OrbitThreeVisualEngine{
  ...
  syncMenu(settings){
    ...
    this.player.visible=this.playerRim.visible=this.playerCore.visible=this.playerShadow.visible=true;this.placeSprite(this.playerShadow,x+4,y+19,66,25,0,1.2,.64,'#020208');this.playerGlow.visible=settings.glow!=='OFF';this.playerReadabilityRing.visible=true;this.playerReadabilityRing.position.set(x,y,2.8);this.playerReadabilityRing.rotation.z=t*.15;this.playerReadabilityRing.scale.set(38+2*pulse,38+2*pulse,1);this.playerReadabilityRing.material.color.set('#35efff');this.playerReadabilityRing.material.opacity=.28;this.placeSprite(this.playerRim,x,y,59,88,yaw,4,.28,'#80f5e1');this.placeSprite(this.player,x,y,56,84,yaw,5,1,'#f5f7f2');this.placeSprite(this.playerCore,x,y-2,18+3*pulse,18+3*pulse,0,6,.48+.15*pulse,'#9ff6df');this.placeSprite(this.playerGlow,x,y,122,122,0,3,.20*pulse,'#35efff');
    this.playerTrail.visible=this.playerAttitudeLeft.visible=this.playerAttitudeRight.visible=false;this.playerEngineLeft.visible=this.playerEngineRight.visible=true;this.targetLock.visible=this.targetLockSweep.visible=false;this.placeSprite(this.playerEngineLeft,x-5,y+48,7,38,Math.PI/2,2,.36+.11*pulse,combatPalette.friendly);this.placeSprite(this.playerEngineRight,x+5,y+48,7,36,Math.PI/2,2,.34+.10*(1-pulse),combatPalette.friendlySoft);
    for(const pool of Object.values(this.pools))for(const item of pool)item.visible=false;
  }

  syncPlayer(state,settings,glow){
    const p=state.p,m=this.playerMotion,frameAccent=p.visualAccent||combatPalette.friendly,frameEdge=p.visualEdge||combatPalette.friendlySoft,neutralAngle=-Math.PI/2,strafeTarget=p.moving?clamp(p._lastMoveX||0,-1,1):0,surgeTarget=p.moving?clamp(-(p._lastMoveY||0),-1,1):0,targetAngle=neutralAngle+strafeTarget*.22*this.motionScale;if(!m.ready){Object.assign(m,{ready:true,x:p.x,y:p.y,angle:neutralAngle,bank:0,strafe:0,surge:0,thrust:0,velocity:0,engineBias:0})}
    ...
    const turnCompression=1-Math.min(.105,Math.abs(m.bank)*.48),surgeStretch=1+m.surge*.04*m.thrust,surgeCompression=1-m.surge*.022*m.thrust,visualYaw=m.angle-neutralAngle;m.visualYaw=visualYaw;this.player.visible=this.playerRim.visible=this.playerCore.visible=this.playerShadow.visible=true;this.placeSprite(this.playerShadow,x+5-m.strafe*3,y+18-hover*.32,64*turnCompression*(1+dash*.09),23*(1-dash*.08),0,1.2,.56+.08*m.thrust,'#020208');this.placeSprite(this.playerRim,x,y,58*turnCompression*surgeCompression*(1+dash*.055),86*(1+dash*.026)*surgeStretch,m.angle+Math.PI/2,4,.25+.11*Math.abs(m.bank)+dash*.10,p.hp/p.maxHp<.3?'#ff526f':frameAccent);this.placeSprite(this.player,x,y,54*turnCompression*surgeCompression*(1+kick*.025+dash*.05)/breath,81*(1-kick*.035+dash*.018)*breath*surgeStretch,m.angle+Math.PI/2,5,1,p.hitFlash>0?'#ffffff':'#f5f7f2');const corePulse=.82+.18*Math.sin(this.time*(3.8+m.thrust*3.2));this.placeSprite(this.playerCore,x,y-2,15+corePulse*5+dash*6,15+corePulse*5+dash*6,0,6,.40+.24*corePulse+dash*.17,p.hp/p.maxHp<.3?'#ff6675':frameAccent);
    const clarity=settings.effectClarity||'HIGH',ringOpacity=clarity==='HIGH'?.38:clarity==='BALANCED'?.25:.14,ringRadius=34+corePulse*2+dash*5;this.playerReadabilityRing.visible=clarity!=='CINEMATIC'||glow;this.playerReadabilityRing.position.set(x,y,2.8);this.playerReadabilityRing.rotation.z=this.time*.16;this.playerReadabilityRing.scale.set(ringRadius,ringRadius,1);this.playerReadabilityRing.material.color.set(p.hp/p.maxHp<.3?'#ff3e68':frameAccent);this.playerReadabilityRing.material.opacity=ringOpacity+.08*dash;
    this.playerGlow.visible=glow;this.placeSprite(this.playerGlow,x,y,98+kick*20+dash*30,98+kick*20+dash*30,0,3,(p.hp/p.maxHp<.3?.25:.17)*pulse+(dash*.10),p.hp/p.maxHp<.3?'#ff3159':frameAccent);
    const backX=-Math.cos(m.angle),backY=-Math.sin(m.angle),sideX=-Math.sin(m.angle),sideY=Math.cos(m.angle),trailLength=30+m.thrust*48+m.surge*13+dash*58,trailX=x+backX*(25+trailLength*.48),trailY=y+backY*(25+trailLength*.48),enginePulse=.84+.11*Math.sin(this.time*25)+.05*Math.sin(this.time*41);
    this.playerTrail.visible=glow;this.placeSprite(this.playerTrail,trailX,trailY,trailLength*1.20,15+m.thrust*6,m.angle,2,(.13+.17*m.thrust+.14*dash)*enginePulse,frameAccent);
    this.playerEngineLeft.visible=this.playerEngineRight.visible=true;for(const [engine,side] of [[this.playerEngineLeft,-1],[this.playerEngineRight,1]]){const output=1-side*m.engineBias,offset=side*(5.5+Math.abs(m.bank)*3),length=trailLength*output,ex=x+backX*(25+length*.48)+sideX*offset,ey=y+backY*(25+length*.48)+sideY*offset,flutter=side<0?enginePulse:1.02-enginePulse*.10;this.placeSprite(engine,ex,ey,length,6+m.thrust*3,m.angle,3,(.40+.50*m.thrust+.20*dash)*flutter*(.92+output*.08),side<0?frameAccent:frameEdge)}
    for(const [thruster,side] of [[this.playerAttitudeLeft,-1],[this.playerAttitudeRight,1]]){const demand=Math.max(0,m.strafe*-side),length=10+demand*23,angle=Math.atan2(sideY*side,sideX*side),tx=x+sideX*side*(19+length*.43),ty=y+sideY*side*(19+length*.43)+m.surge*3;thruster.visible=demand>.025;this.placeSprite(thruster,tx,ty,length,4.5+demand*2,angle,4,(.20+.62*demand)*(.88+.12*Math.sin(this.time*31+side)),frameAccent)}
    const echoes=p.dashFx>0?7:0;this.use(this.pools.echoes,echoes,()=>this.sprite(this.textures.player,THREE.AdditiveBlending),(sprite,i)=>{const t=(i+1)/(echoes+1),fade=1-t,eased=t*t*(3-2*t);this.placeSprite(sprite,p.x+(p.dashFromX-p.x)*eased,p.y+(p.dashFromY-p.y)*eased,54*(.9+fade*.08),81*(.9+fade*.08),m.angle+Math.PI/2,2,.24*fade*dash,combatPalette.friendly)});
  }

  syncEnemies(state,settings,glow){
    const enemies=state.enemies.filter(enemy=>!enemy.dead),clarity=settings.effectClarity||'HIGH',quality=settings.graphics||'HIGH',clarityBoost=clarity==='HIGH'?1.42:clarity==='BALANCED'?1.18:1;
    this.use(this.pools.enemies,enemies.length,()=>this.sprite(this.textures['enemy-scout']),(sprite,i)=>{const enemy=enemies[i],boss=!!enemy.boss,role=boss?'boss':enemy.type,size=enemy.r*(boss?4.25:enemy.elite?3.72:3.42),aimAngle=Math.atan2(state.p.y-enemy.y,state.p.x-enemy.x),m=this.motionFor(sprite,enemy,enemy.x,enemy.y,aimAngle),targetAngle=this.movementHeading(m,enemy.x,enemy.y,aimAngle);this.smoothMotion(m,enemy.x,enemy.y,targetAngle,boss?8:['charger','stalker','reaper','moth'].includes(enemy.type)?19:14,boss?3.8:7.5);m.spawn=clamp(m.spawn+this.delta*(boss?.55:enemy.elite?.85:1.25));const phase=this.time*(boss?.72:['charger','stalker','reaper','moth'].includes(enemy.type)?3.4:enemy.type==='tank'||enemy.type==='warden'||enemy.type==='anchor'?.92:['splitter','weaver','harrower'].includes(enemy.type)?2.35:1.72)+i*.73,breath=Math.sin(phase),hover=Math.sin(phase*.71)*(boss?1.8:enemy.elite?1.0:.62)*this.motionScale,charge=(['charger','stalker','reaper'].includes(enemy.type))&&enemy.burst>0?.14:0,hitKick=clamp((enemy.hit||0)/.08)*4.2,fireRecoil=(['gunner','sniper','weaver','harrower','anchor'].includes(enemy.type)||boss)&&enemy.shootT>0&&enemy.shootT<.18?clamp((.18-enemy.shootT)/.18)*3.4:0,entry=easeOutBack(m.spawn),texture=this.textures[`enemy-${role}`]||this.textures['enemy-scout'],turnCompression=1-Math.min(.09,Math.abs(m.bank)*.55),speedStretch=1+clamp(m.velocity*.00065,0,.075)*this.motionScale,swayScale=boss?.045:enemy.type==='tank'||enemy.type==='warden'||enemy.type==='anchor'?.025:['splitter','weaver','harrower','moth'].includes(enemy.type)?.12:['charger','stalker','reaper'].includes(enemy.type)?.075:.055,organicSway=Math.sin(phase*(['splitter','weaver','harrower','moth'].includes(enemy.type)?1.65:1.12))*enemy.r*swayScale*this.motionScale,sideX=-Math.sin(m.angle),sideY=Math.cos(m.angle),wobble=Math.sin(phase*.91)*swayScale*.42*this.motionScale,tint=enemy.hit>0?'#ffffff':enemy.nemesis?'#f2f4f1':enemy.bounty?'#efc65e':enemyBodyTint[role]||enemyBodyTint.scout;if(sprite.material.map!==texture){sprite.material.map=texture;sprite.material.needsUpdate=true}sprite.userData.enemyRole=role;this.placeSprite(sprite,m.x+sideX*organicSway-Math.cos(m.angle)*(hitKick+fireRecoil),m.y+hover+sideY*organicSway-Math.sin(m.angle)*(hitKick+fireRecoil),size*.67*turnCompression*(1+breath*.045+charge)*entry/Math.sqrt(speedStretch),size*(1-breath*.032-charge*.08)*entry*speedStretch,m.angle+Math.PI/2+wobble,4,easeOutCubic(m.spawn),tint)});
    this.use(this.pools.enemyShadows,quality==='LOW'?0:enemies.length,()=>this.sprite(this.shadowTexture),(shadow,i)=>{const enemy=enemies[i],body=this.pools.enemies[i],priority=enemy.boss||enemy.elite||enemy.nemesis||enemy.bounty,size=enemy.r*(enemy.boss?5.0:enemy.elite?4.15:3.55),motion=body.userData.motion,speed=clamp((motion?.velocity||0)/180);this.placeSprite(shadow,body.position.x+4-(motion?.vx||0)*.008,body.position.y+enemy.r*(enemy.boss?.86:.68),size*(1+speed*.12),size*(enemy.boss?.34:.27),0,1.1,priority?.62:.47,'#020207')});
    this.use(this.pools.enemySilhouettes,enemies.length,()=>this.sprite(this.textures['enemy-scout']),(silhouette,i)=>{const body=this.pools.enemies[i],texture=body.material.map;if(silhouette.material.map!==texture){silhouette.material.map=texture;silhouette.material.needsUpdate=true}silhouette.userData.enemyRole=body.userData.enemyRole;this.placeSprite(silhouette,body.position.x,body.position.y,body.scale.x*1.20,body.scale.y*1.16,body.material.rotation,3.7,.94,'#030104')});
    this.use(this.pools.enemyRims,quality==='LOW'?0:enemies.length,()=>this.sprite(this.textures['enemy-scout'],THREE.AdditiveBlending),(rim,i)=>{const enemy=enemies[i],body=this.pools.enemies[i],role=enemy.boss?'boss':enemy.type,texture=body.material.map,priority=enemy.boss||enemy.elite||enemy.nemesis||enemy.bounty,color=enemy.nemesis?'#e8eeeb':enemy.bounty?'#e0b84f':enemyAccent[role]||enemyAccent.scout;if(rim.material.map!==texture){rim.material.map=texture;rim.material.needsUpdate=true}this.placeSprite(rim,body.position.x-1.4,body.position.y-1.8,body.scale.x*1.055,body.scale.y*1.045,body.material.rotation,4.2,priority?.22:.105,color)});
    this.use(this.pools.enemyWakes,enemies.length,()=>this.sprite(this.textures['enemy-scout'],THREE.AdditiveBlending),(wake,i)=>{const enemy=enemies[i],body=this.pools.enemies[i],m=body.userData.motion,role=enemy.boss?'boss':enemy.type,texture=body.material.map,speed=clamp((m?.velocity||0)/150),distance=4+speed*(enemy.boss?13:9),vx=m?.vx||Math.cos(m?.angle||0),vy=m?.vy||Math.sin(m?.angle||0),length=Math.hypot(vx,vy)||1,color=enemy.nemesis?'#dfe9e5':enemyAccent[role]||enemyAccent.scout;if(wake.material.map!==texture){wake.material.map=texture;wake.material.needsUpdate=true}this.placeSprite(wake,body.position.x-vx/length*distance,body.position.y-vy/length*distance,body.scale.x*(1+speed*.08),body.scale.y*(1+speed*.05),body.material.rotation,3,.045+.16*speed*this.motionScale,color);wake.visible=speed>.035&&this.motionScale>.1});
    this.use(this.pools.enemyGlows,glow?enemies.length:0,()=>this.sprite(this.glowTexture,THREE.AdditiveBlending),(sprite,i)=>{const enemy=enemies[i],body=this.pools.enemies[i],role=enemy.boss?'boss':enemy.type,priority=enemy.boss||enemy.elite||enemy.nemesis||enemy.bounty,size=enemy.r*(enemy.boss?5.4:priority?4.35:3.45),pulse=.88+.12*Math.sin(this.time*(enemy.boss?1.2:2.1)+i),color=enemy.nemesis?'#e8eeeb':enemy.bounty?'#e0b84f':enemyAccent[role]||enemyAccent.scout;this.placeSprite(sprite,body.position.x,body.position.y,size*pulse,size*pulse,0,2,Math.min(.30,(priority?.18:.105)*clarityBoost),color)});
    const marked=enemies.map((enemy,index)=>({enemy,index})).filter(({enemy})=>enemy.boss||enemy.elite||enemy.nemesis||enemy.bounty||clarity==='HIGH'&&markedEnemyTypes.has(enemy.type)||clarity==='BALANCED'&&['charger','gunner','sniper','reaper','harrower','moth','anchor'].includes(enemy.type)),ringBoost=clarity==='HIGH'?1.25:clarity==='BALANCED'?1.10:1;
    this.use(this.pools.enemyRings,marked.length,()=>this.mesh(this.flatMaterial('#ffffff',THREE.AdditiveBlending),new THREE.RingGeometry(.86,1,6)),(mesh,i)=>{const {enemy,index}=marked[i],body=this.pools.enemies[index],role=enemy.boss?'boss':enemy.type,intent=isDashIntent(enemy)||isRangedIntent(enemy),pulse=1+(intent?.10*Math.sin(this.time*15):.025*Math.sin(this.time*3+i)),radius=enemy.r*(enemy.boss?2.65:enemy.elite?2.18:2.05)*pulse,color=enemy.nemesis?'#edf3f0':enemy.bounty?'#ffd36d':enemyAccent[role]||enemyAccent.scout;mesh.position.set(body.position.x,body.position.y,3);mesh.rotation.z=this.time*(enemy.boss?.28:.12)+i*.73;mesh.scale.set(radius,radius,1);mesh.material.color.set(color);mesh.material.opacity=Math.min(.68,(intent?.52:enemy.boss?.42:enemy.elite?.34:.23)*ringBoost);mesh.userData.enemyRole=role});
    this.use(this.pools.enemyMarkers,marked.length,()=>this.sprite(this.enemyRoleTextures.charger),(sprite,i)=>{const {enemy,index}=marked[i],body=this.pools.enemies[index],role=enemy.boss?'boss':enemy.type,texture=this.enemyRoleTextures[role]||this.enemyRoleTextures.charger,intent=isDashIntent(enemy)||isRangedIntent(enemy),size=(enemy.boss?23:enemy.elite?20:clarity==='HIGH'?17:15)*(intent?1.10+.06*Math.sin(this.time*18):1),color=enemy.nemesis?'#f4f6f3':enemy.bounty?'#ffd36d':enemyAccent[role]||enemyAccent.scout;if(sprite.material.map!==texture){sprite.material.map=texture;sprite.material.needsUpdate=true}sprite.userData.enemyRole=role;this.placeSprite(sprite,body.position.x,body.position.y-enemy.r*(enemy.boss?2.55:2.15),size,size,0,8,intent?1:.94,color)});
  }
  ...
  syncProjectiles(state,settings){
    ...
    this.use(this.pools.projectileTrails,state.bullets.length,()=>{const sprite=this.sprite(this.engineTexture,THREE.AdditiveBlending);sprite.center.set(.82,.5);return sprite},(trail,i)=>{const bullet=state.bullets[i],profile=projectileProfile[bullet.weaponId]||projectileProfile.unknown,speed=Math.hypot(bullet.vx||0,bullet.vy||0),angle=projectileHeading(bullet),dx=Math.cos(angle),dy=Math.sin(angle),age=bullet.maxLife?clamp(1-bullet.life/bullet.maxLife):1,launch=easeOutCubic(Math.min(1,age*18)),length=(profile.trail+Math.min(28,speed*.035))*launch,pulse=.90+.10*Math.sin(this.time*34+i*.77),color=projectileAccent[bullet.weaponId]||projectileAccent.unknown;this.placeSprite(trail,bullet.x-dx*length*.42,bullet.y-dy*length*.42,length,profile.trailHeight*pulse,angle,5.5,(bullet.weaponId==='rail'?.50:.31)*clarity*pulse,color)});
    this.use(this.pools.bullets,state.bullets.length,()=>{const sprite=this.sprite(this.projectileTexture,THREE.AdditiveBlending);sprite.center.set(.68,.5);return sprite},(sprite,i)=>{const bullet=state.bullets[i],profile=projectileProfile[bullet.weaponId]||projectileProfile.unknown,speed=Math.hypot(bullet.vx||0,bullet.vy||0),angle=projectileHeading(bullet),age=bullet.maxLife?clamp(1-bullet.life/bullet.maxLife):1,launch=easeOutBack(Math.min(1,age*21)),pulse=.96+.04*Math.sin(this.time*39+i),length=(profile.body+Math.min(17,speed*.022))*launch,height=profile.height*(2-launch*.98)*pulse,color=projectileAccent[bullet.weaponId]||projectileAccent.unknown;this.placeSprite(sprite,bullet.x,bullet.y,length,height,angle,7,profile.opacity,color)});
    this.use(this.pools.bulletSilhouettes,state.bullets.length,()=>{const sprite=this.sprite(this.projectileTexture);sprite.center.set(.68,.5);return sprite},(silhouette,i)=>{const bullet=state.bullets[i],body=this.pools.bullets[i],angle=projectileHeading(bullet);this.placeSprite(silhouette,body.position.x,body.position.y,body.scale.x+6,body.scale.y+4,angle,6,.82,combatPalette.void)});
    this.use(this.pools.hostileProjectileTrails,state.enemyBullets.length,()=>{const sprite=this.sprite(this.engineTexture,THREE.AdditiveBlending);sprite.center.set(.84,.5);return sprite},(trail,i)=>{const bullet=state.enemyBullets[i],angle=projectileHeading(bullet),speed=Math.hypot(bullet.vx||0,bullet.vy||0),length=18+Math.min(22,speed*.055),dx=Math.cos(angle),dy=Math.sin(angle),pulse=.88+.12*Math.sin(this.time*29+i*.41);this.placeSprite(trail,bullet.x-dx*length*.44,bullet.y-dy*length*.44,length,4.2,angle,5.8,.34*clarity*pulse,combatPalette.hostile)});
    this.use(this.pools.hostileBulletOutlines,state.enemyBullets.length,()=>this.mesh(this.flatMaterial(combatPalette.void),new THREE.RingGeometry(.68,1,32)),(mesh,i)=>{const bullet=state.enemyBullets[i],radius=bullet.r+4.8;mesh.position.set(bullet.x,bullet.y,6);mesh.scale.set(radius,radius,1);mesh.material.opacity=settings.effectClarity==='HIGH'?.96:settings.effectClarity==='BALANCED'?.78:.54});
    this.use(this.pools.hostileBulletGlows,state.enemyBullets.length,()=>this.sprite(this.glowTexture,THREE.AdditiveBlending),(sprite,i)=>{const bullet=state.enemyBullets[i],pulse=.9+.1*Math.sin(this.time*27+i),size=(bullet.r+5.2)*2.4*pulse;this.placeSprite(sprite,bullet.x,bullet.y,size,size,0,6.5,.42,combatPalette.hostile)});
    this.use(this.pools.hostileBullets,state.enemyBullets.length,()=>{const sprite=this.sprite(this.projectileTexture,THREE.AdditiveBlending);sprite.center.set(.67,.5);return sprite},(sprite,i)=>{const bullet=state.enemyBullets[i],angle=projectileHeading(bullet),speed=Math.hypot(bullet.vx||0,bullet.vy||0),length=17+Math.min(13,speed*.035),height=Math.max(5.5,bullet.r*1.65),pulse=.94+.06*Math.sin(this.time*31+i*.63);this.placeSprite(sprite,bullet.x,bullet.y,length,height*pulse,angle,7,1,combatPalette.danger)});
  }
  ...
  syncLoot(state,quality){
    ...
    const signalColors={repair:'#72e8a4',flux:'#49dce8',salvage:'#c99bdb',fracture:'#8e7dff',relic:'#d0589f',jammer:'#ff536f'};
    this.use(this.pools.loot,items.length,()=>this.sprite(this.pickupTextures.orb),(sprite,i)=>{const {item,type}=items[i],signal=type==='signal',cache=type==='cache',reliquary=cache&&item.rarity==='RELIQUARY',kind=signal?(item.type||'jammer'):reliquary?'relic':cache?'cache':'orb',texture=this.pickupTextures[kind]||this.pickupTextures.orb,size=signal?(item.disposition==='hazard'?34:31):reliquary?38:cache?26:item.r>4?20:16,color=signal?(signalColors[kind]||item.color):reliquary?'#ff4e9d':cache?(item.rarity==='PARADOX'?'#a98bc0':item.rarity==='OMEGA'?'#e5e0d3':item.rarity==='RARE'?'#c3a362':'#83a7a3'):(item.r>4?'#c1a56f':'#79aa9e'),pulse=.96+(reliquary?.10:.055)*Math.sin(this.time*(reliquary?4.4:signal?2.1:3.2)+i),rotation=kind==='orb'?this.time*.24+i*.31:reliquary?this.time*.42:Math.sin(this.time*.72+i)*.025;if(sprite.material.map!==texture){sprite.material.map=texture;sprite.material.needsUpdate=true}sprite.userData.pickupKind=kind;this.placeSprite(sprite,item.x,item.y+Math.sin(this.time*(reliquary?2.8:signal?1.45:2.2)+i)*((signal?2.2:reliquary?2.0:1.0)*this.motionScale),size*pulse,size*pulse,rotation,6,reliquary?1:.96,color)});
  }
  ...
  syncTransientFx(state,quality,glow,settings){
    ...
    const impacts=state.impactFx||[],explosiveImpacts=impacts.filter(impact=>impact.finalHit||impact.blast>0||['rail','missile','nova','mine','flak'].includes(impact.weaponId)),ringImpacts=explosiveImpacts,sparks=[];
    for(const impact of impacts){const explosive=impact.blast>0||['missile','nova','mine','flak'].includes(impact.weaponId),count=quality==='LOW'?1:impact.finalHit?8:explosive?6:impact.weaponId==='rail'?4:3;for(let index=0;index<count;index++)sparks.push({impact,index,count})}
    ...
    this.use(this.pools.impactBursts,explosiveImpacts.length,()=>this.sprite(this.burstTexture,THREE.AdditiveBlending),(sprite,i)=>{const impact=explosiveImpacts[i],life=clamp(impact.life/impact.maxLife),progress=1-life,blast=Math.min(impact.blast||0,120),size=impact.size*(impact.finalHit?2.8:2.15)+blast*.42,scale=size*(.42+Math.sin(progress*Math.PI)*.72);this.placeSprite(sprite,impact.x,impact.y,scale,scale,(impact.seed||0)*TAU,9.15,life*(impact.finalHit?.82:.62),impact.finalHit?'#ffffff':impact.color)});
    this.use(this.pools.impactRings,ringImpacts.length,()=>this.mesh(this.flatMaterial('#ffffff',THREE.AdditiveBlending),new THREE.RingGeometry(.89,1,64)),(mesh,i)=>{const impact=ringImpacts[i],life=clamp(impact.life/impact.maxLife),progress=1-life,radius=impact.size*(.30+progress*(impact.finalHit?1.55:1.12))+Math.min(impact.blast||0,120)*progress*.48;mesh.position.set(impact.x,impact.y,9);mesh.scale.set(radius,radius,1);mesh.material.color.set(impact.color||'#ffffff');mesh.material.opacity=life*(impact.finalHit?.82:.62)});
    this.use(this.pools.impactSparks,sparks.length,()=>{const sprite=this.sprite(this.engineTexture,THREE.AdditiveBlending);sprite.center.set(.86,.5);return sprite},(sprite,i)=>{const {impact,index,count}=sparks[i],life=clamp(impact.life/impact.maxLife),progress=1-life,radial=impact.blast>0||impact.finalHit,spread=radial?index/count*TAU+(impact.seed||0)*.6:(index-(count-1)/2)*(.28+(impact.seed||0)*.10),angle=radial?spread:impact.angle+spread,length=impact.size*(.54+(index%3)*.18)*(1+progress*.90),x=impact.x-Math.cos(angle)*length*.45,y=impact.y-Math.sin(angle)*length*.45;this.placeSprite(sprite,x,y,length,impact.finalHit?3.6:2.5,angle,9.1,life*(impact.finalHit?.96:.68),impact.finalHit?'#ffffff':impact.color)});
    const telegraphs=settings.telegraphs==='OFF'?[]:[...state.enemies.filter(enemy=>!enemy.dead&&isRangedIntent(enemy,.42)).map(enemy=>({x1:enemy.x,y1:enemy.y,x2:state.p.x,y2:state.p.y,width:enemy.boss?1.8:1.1,alpha:clamp((.42-enemy.shootT)/.42)*.66,color:enemy.boss?'#c83e53':enemy.type==='sniper'?'#c69b59':enemy.type==='weaver'?'#7f89ff':enemy.type==='harrower'?'#d98145':enemy.type==='anchor'?'#647cff':'#ad5d68'})),...state.enemies.filter(enemy=>!enemy.dead&&isDashIntent(enemy)).map(enemy=>{const timer=dashWarningTimer(enemy),active=enemy.burst>0;return{x1:enemy.x,y1:enemy.y,x2:state.p.x,y2:state.p.y,width:active?1.8:1.2,alpha:active?.46:clamp((.58-timer)/.58)*.54,color:enemy.type==='reaper'?'#a7cc56':enemy.type==='stalker'?'#ca62bb':'#df8659'}})];
  ...
  syncDeathFx(state,quality,glow){
    const deaths=state.deathFx||[];this.use(this.pools.deathGlows,glow?deaths.length:0,()=>this.sprite(this.glowTexture,THREE.AdditiveBlending),(sprite,i)=>{const death=deaths[i],life=clamp(death.life/(death.maxLife||.55)),progress=1-life,size=death.r*(death.boss?6.4:death.elite?4.8:3.7)*(1+progress*.8);this.placeSprite(sprite,death.x,death.y,size,size,0,8,life*(death.boss?.42:death.elite?.30:.24),death.color||combatPalette.hostile)});
    this.use(this.pools.deathBursts,quality==='LOW'?deaths.filter(death=>death.elite||death.boss).length:deaths.length,()=>this.sprite(this.burstTexture,THREE.AdditiveBlending),(sprite,i)=>{const death=(quality==='LOW'?deaths.filter(item=>item.elite||item.boss):deaths)[i],life=clamp(death.life/(death.maxLife||.55)),progress=1-life,size=death.r*(death.boss?7.2:death.elite?5.2:3.7)*(.38+Math.sin(progress*Math.PI)*.76);this.placeSprite(sprite,death.x,death.y,size,size,(death.seed||0)*TAU,8.8,life*(death.boss?.88:death.elite?.70:.52),death.color||combatPalette.hostile)});
    this.use(this.pools.deathRings,deaths.length,()=>this.mesh(this.flatMaterial('#ffffff',THREE.AdditiveBlending),new THREE.RingGeometry(.90,1,64)),(mesh,i)=>{const death=deaths[i],life=clamp(death.life/(death.maxLife||.55)),progress=1-life,radius=death.r*(.8+progress*(death.boss?5.4:death.elite?4.0:3.4));mesh.position.set(death.x,death.y,9);mesh.scale.set(radius,radius,1);mesh.material.color.set(death.color||combatPalette.hostile);mesh.material.opacity=life*(death.boss?.90:death.elite?.82:.72)});
  }
  ...
  syncTargetLock(state,settings){
    const target=state.lockedTarget,active=state.enemies.filter(enemy=>!enemy.dead),body=this.pools.enemies[active.indexOf(target)],x=body?.position.x??target?.x,y=body?.position.y??target?.y,visible=!!target&&!target.dead&&x>18&&x<this.width-18&&y>72&&y<this.height-34;this.targetLock.visible=this.targetLockSweep.visible=visible;if(!visible)return;
    const acquire=clamp((state.targetLockPulse||0)/.30),pulse=.5+.5*Math.sin(this.time*5.5),radius=Math.max(19,target.r*1.62)*(1+acquire*.24),color=target.boss?'#ff4e9d':target.elite||target.nemesis||target.bounty?'#ffd27a':'#8de9ff',clarity=settings.effectClarity==='HIGH'?1:settings.effectClarity==='BALANCED'?.78:.58;this.targetLock.position.set(x,y,9.6);this.targetLock.rotation.z=this.time*.46+Math.PI/4;this.targetLock.scale.set(radius,radius,1);this.targetLock.material.color.set(color);this.targetLock.material.opacity=(.34+.22*pulse+.28*acquire)*clarity;this.targetLock.userData.target=target;this.targetLockSweep.position.set(x,y,9.7);this.targetLockSweep.rotation.z=-this.time*.82;this.targetLockSweep.scale.set(radius*.78,radius*.78,1);this.targetLockSweep.material.color.set(color);this.targetLockSweep.material.opacity=(.22+.22*acquire)*clarity;
  }
