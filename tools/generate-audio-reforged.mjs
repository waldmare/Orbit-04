...
await save('pulse_alt',.32,(t)=>{const e=attackRelease(t,.32,.0005,.22),snap=noise()*Math.exp(-t*86),coil=chirp(t,.32,760,210)*Math.exp(-t*12),body=sine(t,61)*Math.exp(-t*17);return[e*(snap*.18+coil*.48+body*.40),e*(snap*.13+coil*.40+body*.44)]},{space:.11,gain:.84,lowpass:.34});
await save('system_install',1.18,(t)=>{const latch=noise()*Math.exp(-t*96),body=sine(t,76)*Math.exp(-t*7),charge=chirp(t,1.18,118,920)*Math.exp(-t*3.7),seal=sine(t,311,.4)*Math.exp(-Math.max(0,t-.34)*9)*(t>.34?1:0);return[latch*.44+body*.52+charge*.23+seal*.16,latch*.31+body*.57+charge*.18-seal*.13]},{space:.38,gain:.90,lowpass:.27});
await save('flak',.64,(t)=>{const burst=(offset,pitch)=>{const q=t-offset;if(q<0)return 0;return noise()*Math.exp(-q*88)+sine(q,pitch)*Math.exp(-q*18)};const sub=sine(t,47)*Math.exp(-t*7),debris=noise()*Math.exp(-t*5.8);return[burst(0,118)*.42+burst(.045,94)*.34+burst(.092,137)*.27+sub*.55+debris*.14,burst(0,111)*.34+burst(.052,101)*.39+burst(.10,129)*.23+sub*.59-debris*.12]},{space:.25,gain:.93,lowpass:.31});
await save('rift',2.42,(t)=>{const x=t/2.42,e=attackRelease(t,2.42,.018,.72),fold=chirp(t,2.42,72,1240)*Math.sin(Math.PI*x)*.26,sub=(sine(t,33-9*x)+sine(t,49,.7)*.35)*.52,voidNoise=noise()*(.05+.18*Math.sin(Math.PI*x)),ring=sine(t,178+sine(t,4.2)*24)*Math.exp(-t*1.7)*.16;return[e*(fold+sub+voidNoise+ring),e*(fold*.70+sub*1.04-voidNoise*.86-ring*.75)]},{space:.78,gain:.95,lowpass:.14});
await save('enemy_heavy_shot',.98,(t)=>{const e=attackRelease(t,.98,.001,.38),rupture=noise()*Math.exp(-t*68),throat=(sine(t,71+sine(t,18)*12)+sine(t,109,.8)*.35)*Math.exp(-t*5.5),spit=chirp(t,.98,460,64)*Math.exp(-t*7),tail=noise()*Math.exp(-t*3.8);return[e*(rupture*.39+throat*.52+spit*.28+tail*.10),e*(rupture*.31+throat*.57-spit*.22-tail*.08)]},{space:.31,gain:.91,lowpass:.22});
await save('sub_bass',1.14,(t)=>{const e=attackRelease(t,1.14,.001,.48),fund=sine(t,42-13*t/1.14),harm=sine(t,84,.5)*Math.exp(-t*7),air=noise()*Math.exp(-t*28);return[e*(fund*.86+harm*.16+air*.025),e*(fund*.88+harm*.13-air*.02)]},{space:.04,gain:.94,lowpass:.10});
await save('achievement',2.36,(t)=>{const e=attackRelease(t,2.36,.004,.66),notes=[110,164.81,220,329.63],tones=notes.reduce((sum,f,j)=>{const q=t-j*.21;return sum+(q>=0?(sine(q,f)+sine(q,f*2,.4)*.24)*Math.exp(-q*2.9):0)},0),sub=sine(t,36.7)*Math.exp(-t*1.6),metal=noise()*Math.exp(-Math.max(0,t-.64)*8)*(t>.64?1:0);return[e*(tones*.18+sub*.42+metal*.035),e*(tones*.16+sub*.45-metal*.028)]},{space:.68,gain:.88,lowpass:.23});
await save('artifact',2.72,(t)=>{const e=attackRelease(t,2.72,.012,.78),x=t/2.72,drone=(sine(t,46.25)+sine(t,69.38,.8)*.47+sine(t,92.5,.3)*.21)*.34,reverse=chirp(t,2.72,88,1760)*Math.pow(Math.sin(Math.PI*x),1.4)*.20,whisper=noise()*Math.sin(Math.PI*x)*.10,pulse=sine(t,29)*(.14+.12*Math.sin(t*5.4));return[e*(drone+reverse+whisper+pulse),e*(drone*.96-reverse*.74-whisper*.86+pulse*1.04)]},{space:.84,gain:.91,lowpass:.15});
await save('graze',.27,(t)=>{const e=attackRelease(t,.27,.0004,.16),air=noise()*Math.sin(Math.PI*t/.27),pass=chirp(t,.27,420,2100)*.23,body=sine(t,120)*Math.exp(-t*18);return[e*(air*.15+pass+body*.12),e*(-air*.13+pass*.72+body*.14)]},{space:.30,gain:.72,lowpass:.42});
await save('streak',.88,(t)=>{const e=attackRelease(t,.88,.001,.35),hit=noise()*Math.exp(-t*92),chord=[110,164.81,220].reduce((s,f)=>s+sine(t,f)*Math.exp(-t*5),0),sub=sine(t,41)*Math.exp(-t*4.2);return[e*(hit*.32+chord*.19+sub*.48),e*(hit*.25+chord*.17+sub*.52)]},{space:.36,gain:.88,lowpass:.25});
await save('engine_loop',8,(t)=>{const breathe=.82+.18*sine(t,.125),core=(sine(t,41)+sine(t,82,.6)*.31+sine(t,123,.2)*.12)*.24*breathe,turbulence=(sine(t,13.125+sine(t,.25)*1.4)+sine(t,19.25,.8)*.55)*.055,servo=sine(t,246+sine(t,.375)*9)*.018;return[core+turbulence+servo,core*1.02-turbulence*.88-servo*.72]},{space:.24,gain:.64,loop:true,lowpass:.12});
