...
(()=>{
  ...
  class OrbitVisualEngine{
    ...
    drawEnergyEffects(s,settings,quality,glowOn){
      ...
      if(settings.telegraphs!=='OFF')for(const e of s.enemies){if(e.dead)continue;const ranged=e.type==='gunner'||e.type==='sniper'||e.type==='weaver'||e.type==='anchor'||e.boss;if(ranged&&e.shootT>0&&e.shootT<.34){const alpha=(.34-e.shootT)/.34,color=e.boss?'#ff526f':e.type==='sniper'?'#ffcf78':e.type==='weaver'?'#8993ff':e.type==='anchor'?'#657fff':'#ff8ca1';if(glowOn)line(e.x,e.y,s.p.x,s.p.y,color,alpha*.18,5);line(e.x,e.y,s.p.x,s.p.y,color,alpha*.68,1)}if((e.type==='charger'||e.type==='stalker')&&e.burst<=0&&e.chargeT>0&&e.chargeT<.48){const alpha=(.48-e.chargeT)/.48;this.fx.lineStyle(2,0xffc36c,.35+alpha*.5);this.fx.strokeCircle(e.x,e.y,e.r+8+alpha*9)}}
    ...
    drawDangerReadability(s,settings){
      ...
      for(const e of s.enemies||[]){if(e.dead)continue;const ranged=e.type==='gunner'||e.type==='sniper'||e.type==='weaver'||e.type==='anchor'||e.boss;if(ranged&&e.shootT>0&&e.shootT<.42){const alpha=clamp((.42-e.shootT)/.42),color=e.boss?0xff3657:e.type==='sniper'?0xffc35c:e.type==='weaver'?0x8993ff:e.type==='anchor'?0x657fff:0xff6e85;this.dangerFx.lineStyle(high?4:3,0x020307,.62*alpha).beginPath().moveTo(e.x,e.y).lineTo(p.x,p.y).strokePath();this.dangerFx.lineStyle(high?1.5:1.1,color,.82*alpha).beginPath().moveTo(e.x,e.y).lineTo(p.x,p.y).strokePath();this.dangerFx.lineStyle(1.2,color,.48*alpha).strokeCircle(p.x,p.y,15+alpha*8)}if((e.type==='charger'||e.type==='stalker')&&e.burst<=0&&e.chargeT>0&&e.chargeT<.58){const alpha=clamp((.58-e.chargeT)/.58);this.dangerFx.lineStyle(4,0x020307,.62*alpha).strokeCircle(e.x,e.y,e.r+10+alpha*10);this.dangerFx.lineStyle(1.6,0xffbd61,.90*alpha).strokeCircle(e.x,e.y,e.r+10+alpha*10)}if(e.type==='warden'){this.dangerFx.lineStyle(1.3,0x55d1ae,.34).strokeCircle(e.x,e.y,118)}}
    }
