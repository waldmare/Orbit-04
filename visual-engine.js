...
(()=>{
  ...
  const enemyTint={
    scout:0xd27f9a,charger:0xdb876c,tank:0xb89a67,gunner:0xd66e96,splitter:0xba7290,sniper:0xa28bd0,stalker:0xd86cc5,weaver:0x8993df,warden:0x74bba6,reaper:0xa7c966,harrower:0xd99563,moth:0xd883e3,anchor:0x8497e5,boss:0xe36d86
  };
  const enemyTexture={scout:'enemy-scout-v3',charger:'enemy-charger-v3',tank:'enemy-tank-v3',gunner:'enemy-gunner-v3',splitter:'enemy-splitter-v3',sniper:'enemy-sniper-v3',stalker:'enemy-stalker-v3',weaver:'enemy-weaver-v3',warden:'enemy-warden-v3',reaper:'enemy-reaper-v3',harrower:'enemy-harrower-v3',moth:'enemy-moth-v3',anchor:'enemy-anchor-v3',boss:'boss-carrier-v3'};
  ...
  class OrbitVisualEngine{
    ...
    sync(gameState,settings={}){
      ...
      this.syncLoot(gameState);this.syncOrbitals(gameState);this.syncParticles(gameState,particlesOn,clarity);this.syncFloaters(gameState);this.drawEnergyEffects(gameState,settings,quality,glowOn);this.drawTargetLock(gameState,settings);this.syncDeathFx(gameState,quality,glowOn);this.drawDangerReadability(gameState,settings);
    }
    ...
    syncLoot(s){
      ...
      this.use(this.loot,items.length,(v,i)=>{const {entity,x,type}=items[i],cache=type==='cache',signal=type==='signal',reliquary=cache&&x.rarity==='RELIQUARY',col=signal?tintValue(x.color):reliquary?0xff4e9d:cache?(x.rarity==='PARADOX'?0xbd9cff:x.rarity==='OMEGA'?0xffffff:x.rarity==='RARE'?0xffd27a:0x8de9ff):(x.r>4?0xffd27a:0x62f4dd),size=signal?(x.disposition==='hazard'?34:29):reliquary?36:cache?23:(x.r>4?15:10),m=v.motion;this.resetMotion(v,entity,x.x,x.y,0);m.x=smoothValue(m.x,x.x,signal?12:20,this.delta);m.y=smoothValue(m.y,x.y,signal?12:20,this.delta);m.spawn=clamp(m.spawn+this.delta*(signal?1.35:reliquary?1.8:2.8));const hover=Math.sin(this.time*(reliquary?2.8:signal?1.35:2.2)+m.id)*((signal?2.8:reliquary?2:1.2)*this.motionScale),entry=easeOutBack(m.spawn),spin=this.time*(reliquary?.42:signal?.42:cache?.72:1.18)+(x.x||0)*.01;v.root.setPosition(m.x,m.y+hover).setAlpha(easeOutCubic(m.spawn)).setDisplaySize(size*entry,size*entry).setTint(col).setRotation(spin)});
    }
    syncOrbitals(s){
    ...
    drawTargetLock(s,settings){
      const e=s.lockedTarget,index=s.enemies?.indexOf(e)??-1,body=this.enemies?.items?.[index]?.root,x=body?.x??e?.x,y=body?.y??e?.y;if(!e||e.dead||x<18||x>this.width-18||y<72||y>this.height-34)return;const acquire=clamp((s.targetLockPulse||0)/.30),pulse=.5+.5*Math.sin(this.time*5.5),radius=Math.max(19,e.r*1.62)*(1+acquire*.24),color=e.boss?0xff4e9d:e.elite||e.nemesis||e.bounty?0xffd27a:0x8de9ff,clarity=settings.effectClarity==='HIGH'?1:settings.effectClarity==='BALANCED'?.78:.58;this.fx.lineStyle(5,0x020307,.38*clarity).strokeCircle(x,y,radius);this.fx.lineStyle(1.25,color,(.30+.22*pulse+.28*acquire)*clarity).strokeCircle(x,y,radius);for(let i=0;i<4;i++){const a=this.time*.46+i*Math.PI/2,c=Math.cos(a),sn=Math.sin(a);this.strokeFxLine(x+c*(radius-4),y+sn*(radius-4),x+c*(radius+6+acquire*4),y+sn*(radius+6+acquire*4),color,(.48+.28*acquire)*clarity,1.5)}
    }
    drawEnergyEffects(s,settings,quality,glowOn){
      ...
      if(glowOn){
        ...
        for(const cache of s.caches){const reliquary=cache.rarity==='RELIQUARY',color=reliquary?0xff4e9d:cache.rarity==='PARADOX'?0xbd9cff:cache.rarity==='OMEGA'?0xffffff:cache.rarity==='RARE'?0xffd27a:0x8de9ff,pulse=.72+.28*Math.sin(this.time*(reliquary?4.4:3.2)+cache.x*.02),radius=reliquary?27:18;this.fx.fillStyle(color,(reliquary?.12:.07)*pulse).fillCircle(cache.x,cache.y,radius+pulse*3);this.fx.lineStyle(reliquary?2:1.2,color,(reliquary?.46:.22)*pulse).strokeCircle(cache.x,cache.y,(reliquary?19:13)+pulse*2)}
      }
      ...
      if(settings.telegraphs!=='OFF')for(const e of s.enemies){if(e.dead)continue;const ranged=e.boss||rangedThreatTypes.has(e.type);if(ranged&&e.shootT>0&&e.shootT<.34){const alpha=(.34-e.shootT)/.34,color=e.boss?'#ff526f':e.type==='sniper'?'#ffcf78':e.type==='weaver'?'#8993ff':e.type==='harrower'?'#ff9a52':e.type==='anchor'?'#657fff':'#ff8ca1';if(glowOn)line(e.x,e.y,s.p.x,s.p.y,color,alpha*.18,5);line(e.x,e.y,s.p.x,s.p.y,color,alpha*.68,1)}if(isDashIntent(e)){const timer=dashWarningTimer(e),active=e.burst>0,alpha=active?.82:clamp((.58-timer)/.58),color=e.type==='reaper'?0xbdf34b:e.type==='stalker'?0xed48c9:0xff9a63;this.fx.lineStyle(active?3:2,color,.35+alpha*.5);this.fx.strokeCircle(e.x,e.y,e.r+8+alpha*9);line(e.x,e.y,s.p.x,s.p.y,`#${color.toString(16).padStart(6,'0')}`,active?.38:alpha*.26,active?2:1)}}
    ...
    drawDangerReadability(s,settings){
      ...
      for(const e of s.enemies||[]){if(e.dead)continue;const ranged=e.boss||rangedThreatTypes.has(e.type);if(ranged&&e.shootT>0&&e.shootT<.42){const alpha=clamp((.42-e.shootT)/.42),color=e.boss?0xff3657:e.type==='sniper'?0xffc35c:e.type==='weaver'?0x8993ff:e.type==='harrower'?0xff9a52:e.type==='anchor'?0x657fff:0xff6e85;this.dangerFx.lineStyle(high?4:3,0x020307,.62*alpha).beginPath().moveTo(e.x,e.y).lineTo(p.x,p.y).strokePath();this.dangerFx.lineStyle(high?1.5:1.1,color,.82*alpha).beginPath().moveTo(e.x,e.y).lineTo(p.x,p.y).strokePath();this.dangerFx.lineStyle(1.2,color,.48*alpha).strokeCircle(p.x,p.y,15+alpha*8)}if(isDashIntent(e)){const timer=dashWarningTimer(e),active=e.burst>0,alpha=active?.86:clamp((.58-timer)/.58),color=e.type==='reaper'?0xbdf34b:e.type==='stalker'?0xed48c9:0xffbd61;this.dangerFx.lineStyle(high?4:3,0x020307,.58*alpha).beginPath().moveTo(e.x,e.y).lineTo(p.x,p.y).strokePath();this.dangerFx.lineStyle(active?1.8:1.3,color,.78*alpha).beginPath().moveTo(e.x,e.y).lineTo(p.x,p.y).strokePath();this.dangerFx.lineStyle(4,0x020307,.62*alpha).strokeCircle(e.x,e.y,e.r+10+alpha*10);this.dangerFx.lineStyle(1.6,color,.90*alpha).strokeCircle(e.x,e.y,e.r+10+alpha*10)}if(e.type==='warden'){this.dangerFx.lineStyle(1.3,0x55d1ae,.34).strokeCircle(e.x,e.y,118)}}
    }
